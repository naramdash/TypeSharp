...
| -------- | ----------- | -------------------------------- | --------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q1       | Ready       | User Task Inbox | 0401 GitHub Actions regression npm missing in VS Code live smoke       | TBD                                                                                                                                                                        | Investigate the regression workflow failure where `Run shard runners in parallel` reports `npm` missing for `vscode\typesharp`; use the GitHub Actions failure context, keep unrelated workflow changes out, and follow the CI-fix approval workflow before implementation if external check inspection is needed.                                           |
| Q1       | Ready       | Task 0400 null-conditional assignment imported indexer targets | 0402 Roadmap refresh after null-conditional assignment imported indexer targets       | TBD                                                                                                                                                                        | Recheck official C#/F#/TypeScript/.NET Framework/NuGet/.NET testing/MSTest SDK/xUnit.net/VS Code/GitHub Actions signals after `receiver?[index] = value` landed, confirm no `net48`/C# 7.3/package/test-host baseline change, and select the next bounded implementation slice after the user-requested CI regression task.                                           |
