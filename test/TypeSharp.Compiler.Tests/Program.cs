...
var tests = new (string Name, Action Body)[]
{
    ...
    ("checker reports no matching C# delegate lambda binary value return overload diagnostics", CheckerReportsNoMatchingCSharpDelegateLambdaBinaryValueReturnOverloadDiagnostics),
    ("checker reports no matching C# delegate lambda nameof return overload diagnostics", CheckerReportsNoMatchingCSharpDelegateLambdaNameofReturnOverloadDiagnostics),
    ("checker reports no matching C# delegate lambda unchecked return overload diagnostics", CheckerReportsNoMatchingCSharpDelegateLambdaUncheckedReturnOverloadDiagnostics),
    ("checker reports no matching C# delegate lambda satisfies return overload diagnostics", CheckerReportsNoMatchingCSharpDelegateLambdaSatisfiesReturnOverloadDiagnostics),
    ("checker reports no matching C# delegate lambda parenthesized return overload diagnostics", CheckerReportsNoMatchingCSharpDelegateLambdaParenthesizedReturnOverloadDiagnostics),
    ("checker reports no matching C# delegate lambda logical not return overload diagnostics", CheckerReportsNoMatchingCSharpDelegateLambdaLogicalNotReturnOverloadDiagnostics),
    ...
    ("C# overload resolver filters lambda delegate binary value return type", CSharpOverloadResolverFiltersLambdaDelegateBinaryValueReturnType),
    ("C# overload resolver filters lambda delegate nameof return type", CSharpOverloadResolverFiltersLambdaDelegateNameofReturnType),
    ("C# overload resolver filters lambda delegate checked return type", CSharpOverloadResolverFiltersLambdaDelegateCheckedReturnType),
    ("C# overload resolver filters lambda delegate unchecked return type", CSharpOverloadResolverFiltersLambdaDelegateUncheckedReturnType),
    ("C# overload resolver filters lambda delegate satisfies return type", CSharpOverloadResolverFiltersLambdaDelegateSatisfiesReturnType),
    ("C# overload resolver filters lambda delegate parenthesized return type", CSharpOverloadResolverFiltersLambdaDelegateParenthesizedReturnType),
    ("C# overload resolver filters lambda delegate logical not return type", CSharpOverloadResolverFiltersLambdaDelegateLogicalNotReturnType),
    ...
    ("CLI build stops before emission on no matching C# delegate lambda binary value return overload", CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaBinaryValueReturnOverload),
    ("CLI build stops before emission on no matching C# delegate lambda nameof return overload", CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaNameofReturnOverload),
    ("CLI build stops before emission on no matching C# delegate lambda unchecked return overload", CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaUncheckedReturnOverload),
    ("CLI build stops before emission on no matching C# delegate lambda satisfies return overload", CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaSatisfiesReturnOverload),
    ("CLI build stops before emission on no matching C# delegate lambda parenthesized return overload", CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaParenthesizedReturnOverload),
    ("CLI build stops before emission on no matching C# delegate lambda logical not return overload", CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaLogicalNotReturnOverload),
    ...
    ("CLI build compiles parenthesized expression lowering", CliBuildCompilesParenthesizedExpressionLowering),
    ...
    ("CLI build compiles imported delegate lambda overload binary value return match", CliBuildCompilesImportedDelegateLambdaOverloadBinaryValueReturnMatch),
    ("CLI build compiles imported delegate lambda overload nameof return match", CliBuildCompilesImportedDelegateLambdaOverloadNameofReturnMatch),
    ("CLI build compiles imported delegate lambda overload checked return match", CliBuildCompilesImportedDelegateLambdaOverloadCheckedReturnMatch),
    ("CLI build compiles imported delegate lambda overload satisfies return match", CliBuildCompilesImportedDelegateLambdaOverloadSatisfiesReturnMatch),
    ("CLI build compiles imported delegate lambda overload parenthesized return match", CliBuildCompilesImportedDelegateLambdaOverloadParenthesizedReturnMatch),
    ("CLI build compiles imported delegate lambda overload logical not return match", CliBuildCompilesImportedDelegateLambdaOverloadLogicalNotReturnMatch),
...
static void MetadataReaderIndexesLocalPublicSymbols()
{
    WithWorkspace(root =>
    {
        ...
        var delegateBinaryValueReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickBinaryValueReturn").ToArray();
        AssertEqual(2, delegateBinaryValueReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<Legacy.Tools.LegacyNamed, int>", "System.Func`2<Legacy.Tools.LegacyNamed, string>"],
            delegateBinaryValueReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateNameofReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickNameofReturn").ToArray();
        AssertEqual(2, delegateNameofReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<Legacy.Tools.LegacyNamed, int>", "System.Func`2<Legacy.Tools.LegacyNamed, string>"],
            delegateNameofReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateCheckedReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickCheckedReturn").ToArray();
        AssertEqual(2, delegateCheckedReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<string, int>", "System.Func`2<string, string>"],
            delegateCheckedReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateUncheckedReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickUncheckedReturn").ToArray();
        AssertEqual(2, delegateUncheckedReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<string, int>", "System.Func`2<string, string>"],
            delegateUncheckedReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateSatisfiesReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickSatisfiesReturn").ToArray();
        AssertEqual(2, delegateSatisfiesReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<string, int>", "System.Func`2<string, string>"],
            delegateSatisfiesReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateParenthesizedReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickParenthesizedReturn").ToArray();
        AssertEqual(2, delegateParenthesizedReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<string, int>", "System.Func`2<string, string>"],
            delegateParenthesizedReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateLogicalNotReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickLogicalNotReturn").ToArray();
        AssertEqual(2, delegateLogicalNotReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<bool, bool>", "System.Func`2<bool, string>"],
            delegateLogicalNotReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateUnaryNumericReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickUnaryNumericReturn").ToArray();
        AssertEqual(2, delegateUnaryNumericReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<int, int>", "System.Func`2<int, string>"],
            delegateUnaryNumericReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateIfReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickIfReturn").ToArray();
        AssertEqual(2, delegateIfReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<string, int>", "System.Func`2<string, string>"],
            delegateIfReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateBlockReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickBlockReturn").ToArray();
        AssertEqual(2, delegateBlockReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<string, int>", "System.Func`2<string, string>"],
        ...
        AssertSequence(
            ["System.Func`2<Legacy.Tools.LegacyNamed, int>", "System.Func`2<Legacy.Tools.LegacyNamed, string>"],
            delegateCoalesceReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        var delegateIndexerReturnPicks = legacyDelegateOverloads.Methods.Where(method => method.Name == "PickIndexerReturn").ToArray();
        AssertEqual(2, delegateIndexerReturnPicks.Length);
        AssertSequence(
            ["System.Func`2<Legacy.Tools.LegacyFormatter, int>", "System.Func`2<Legacy.Tools.LegacyFormatter, string>"],
            delegateIndexerReturnPicks.Select(method => method.Parameters[1].Type).OrderBy(type => type, StringComparer.Ordinal).ToArray());
        ...
        var requiresBinaryValueReturnInt = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresBinaryValueReturnInt"), "RequiresBinaryValueReturnInt metadata should be present.");
        AssertSequence(["value", "transform"], requiresBinaryValueReturnInt.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["Legacy.Tools.LegacyNamed", "System.Func`2<Legacy.Tools.LegacyNamed, int>"], requiresBinaryValueReturnInt.Parameters.Select(parameter => parameter.Type).ToArray());
        var requiresNameofReturnInt = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresNameofReturnInt"), "RequiresNameofReturnInt metadata should be present.");
        AssertSequence(["value", "transform"], requiresNameofReturnInt.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["Legacy.Tools.LegacyNamed", "System.Func`2<Legacy.Tools.LegacyNamed, int>"], requiresNameofReturnInt.Parameters.Select(parameter => parameter.Type).ToArray());
        var requiresUncheckedReturnString = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresUncheckedReturnString"), "RequiresUncheckedReturnString metadata should be present.");
        AssertSequence(["value", "transform"], requiresUncheckedReturnString.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["string", "System.Func`2<string, string>"], requiresUncheckedReturnString.Parameters.Select(parameter => parameter.Type).ToArray());
        var requiresSatisfiesReturnInt = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresSatisfiesReturnInt"), "RequiresSatisfiesReturnInt metadata should be present.");
        AssertSequence(["value", "transform"], requiresSatisfiesReturnInt.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["string", "System.Func`2<string, int>"], requiresSatisfiesReturnInt.Parameters.Select(parameter => parameter.Type).ToArray());
        var requiresParenthesizedReturnInt = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresParenthesizedReturnInt"), "RequiresParenthesizedReturnInt metadata should be present.");
        AssertSequence(["value", "transform"], requiresParenthesizedReturnInt.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["string", "System.Func`2<string, int>"], requiresParenthesizedReturnInt.Parameters.Select(parameter => parameter.Type).ToArray());
        var requiresLogicalNotReturnString = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresLogicalNotReturnString"), "RequiresLogicalNotReturnString metadata should be present.");
        AssertSequence(["value", "transform"], requiresLogicalNotReturnString.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["bool", "System.Func`2<bool, string>"], requiresLogicalNotReturnString.Parameters.Select(parameter => parameter.Type).ToArray());
        var requiresUnaryNumericReturnString = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresUnaryNumericReturnString"), "RequiresUnaryNumericReturnString metadata should be present.");
        AssertSequence(["value", "transform"], requiresUnaryNumericReturnString.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["int", "System.Func`2<int, string>"], requiresUnaryNumericReturnString.Parameters.Select(parameter => parameter.Type).ToArray());
        var requiresIfReturnInt = Require(legacyDelegateOverloads.Methods.SingleOrDefault(method => method.Name == "RequiresIfReturnInt"), "RequiresIfReturnInt metadata should be present.");
        AssertSequence(["value", "transform"], requiresIfReturnInt.Parameters.Select(parameter => parameter.Name).ToArray());
        AssertSequence(["string", "System.Func`2<string, int>"], requiresIfReturnInt.Parameters.Select(parameter => parameter.Type).ToArray());
...
static void CheckerReportsNoMatchingCSharpDelegateLambdaBinaryValueReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaBinaryValueReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaBinaryValueReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaBinaryValueReturnOverload

            import { LegacyDelegateOverloads, LegacyNamed } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresBinaryValueReturnInt(LegacyNamed("Ada"), item => item.Name + "!")
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda binary value return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresBinaryValueReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaNameofReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaNameofReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaNameofReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaNameofReturnOverload

            import { LegacyDelegateOverloads, LegacyNamed } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresNameofReturnInt(LegacyNamed("Ada"), item => nameof(item.Name))
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda nameof return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresNameofReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaUncheckedReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaUncheckedReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaUncheckedReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaUncheckedReturnOverload

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresUncheckedReturnString("Ada", text => unchecked(1 + 1))
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda unchecked return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresUncheckedReturnString", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaSatisfiesReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaSatisfiesReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaSatisfiesReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaSatisfiesReturnOverload

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresSatisfiesReturnInt("Ada", text => text satisfies string)
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda satisfies return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresSatisfiesReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaParenthesizedReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaParenthesizedReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaParenthesizedReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaParenthesizedReturnOverload

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresParenthesizedReturnInt("Ada", text => (text))
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda parenthesized return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresParenthesizedReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaLogicalNotReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaLogicalNotReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaLogicalNotReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaLogicalNotReturnOverload

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresLogicalNotReturnString(true, flag => !flag)
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda logical-not return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresLogicalNotReturnString", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaUnaryNumericReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaUnaryNumericReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaUnaryNumericReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaUnaryNumericReturnOverload

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresUnaryNumericReturnString(3, value => -value)
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda unary numeric return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresUnaryNumericReturnString", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaIfReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaIfReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaIfReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaIfReturnOverload

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresIfReturnInt("Ada", text => if text == "Ada" { "match" } else { "fallback" })
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda if-expression return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresIfReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaBlockReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaBlockReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaBlockReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaBlockReturnOverload

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresBlockReturnInt("Ada", text => { (text) })
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda block return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresBlockReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}
...
static void CheckerReportsNoMatchingCSharpDelegateLambdaCoalesceReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        ...
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaCoalesceReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaCoalesceReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaCoalesceReturnOverload

            import { LegacyDelegateOverloads, LegacyNamed } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresCoalesceReturnInt(LegacyNamed("Ada"), item => item.Name ?? "fallback")
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda coalesce return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresCoalesceReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}

static void CheckerReportsNoMatchingCSharpDelegateLambdaIndexerReturnOverloadDiagnostics()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaIndexerReturnOverload"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaIndexerReturnOverload"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaIndexerReturnOverload

            import { LegacyDelegateOverloads, LegacyFormatter } from "Legacy.Tools"

            export fun broken(): string =
              LegacyDelegateOverloads.RequiresIndexerReturnInt(LegacyFormatter("legacy:"), item => item[0])
            """);

        var result = TypeSharpChecker.Check(manifestPath);

        AssertTrue(result.HasErrors, "C# delegate lambda indexer return mismatch should produce diagnostics.");
        var diagnostic = result.Diagnostics.Single(diagnostic => diagnostic.Code == "TS2406");
        AssertEqual("src/Main.tysh", diagnostic.File);
        AssertContains("LegacyDelegateOverloads.RequiresIndexerReturnInt", diagnostic.Message);
        AssertContains("matches no overload candidate", diagnostic.Message);
    });
}
...
static void CSharpOverloadResolverFiltersLambdaDelegateBinaryValueReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickBinaryValueReturn(LegacyNamed("Ada"), item => item.Name + "!")
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda binary value expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate binary value return overload candidate.");
    AssertEqual("System.Func`2<Legacy.Tools.LegacyNamed, string>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateNameofReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickNameofReturn(LegacyNamed("Ada"), item => nameof(item.Name))
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda nameof expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate nameof return overload candidate.");
    AssertEqual("System.Func`2<Legacy.Tools.LegacyNamed, string>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateCheckedReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickCheckedReturn("Ada", text => checked(1 + 1))
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda checked expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate checked return overload candidate.");
    AssertEqual("System.Func`2<string, int>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateUncheckedReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickUncheckedReturn("Ada", text => unchecked(1 + 1))
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda unchecked expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate unchecked return overload candidate.");
    AssertEqual("System.Func`2<string, int>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateSatisfiesReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickSatisfiesReturn("Ada", text => text satisfies string)
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda satisfies expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate satisfies return overload candidate.");
    AssertEqual("System.Func`2<string, string>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateParenthesizedReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickParenthesizedReturn("Ada", text => (text))
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda parenthesized expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate parenthesized return overload candidate.");
    AssertEqual("System.Func`2<string, string>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateLogicalNotReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickLogicalNotReturn(true, flag => !flag)
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    var metadataType = new MetadataTypeSymbol(
        ...
        [
            new MetadataMethodSymbol(
                "PickLogicalNotReturn",
                "string",
                MetadataNullabilityKind.NotApplicable,
                [
                    new MetadataParameterSymbol("value", "bool", MetadataByRefKind.None, IsParams: false, IsOptional: false),
                    new MetadataParameterSymbol("transform", "System.Func`2<bool, bool>", MetadataByRefKind.None, IsParams: false, IsOptional: false)
                ]),
            new MetadataMethodSymbol(
                "PickLogicalNotReturn",
                "string",
                ...
                [
                    new MetadataParameterSymbol("value", "bool", MetadataByRefKind.None, IsParams: false, IsOptional: false),
                    new MetadataParameterSymbol("transform", "System.Func`2<bool, string>", MetadataByRefKind.None, IsParams: false, IsOptional: false)
                ])
        ],
        [],
        [],
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda logical-not expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate logical-not return overload candidate.");
    AssertEqual("System.Func`2<bool, bool>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateUnaryNumericReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickUnaryNumericReturn(3, value => +value)
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    var metadataType = new MetadataTypeSymbol(
        ...
        [
            new MetadataMethodSymbol(
                "PickUnaryNumericReturn",
                "string",
                MetadataNullabilityKind.NotApplicable,
                [
                    new MetadataParameterSymbol("value", "int", MetadataByRefKind.None, IsParams: false, IsOptional: false),
                    new MetadataParameterSymbol("transform", "System.Func`2<int, int>", MetadataByRefKind.None, IsParams: false, IsOptional: false)
                ]),
            new MetadataMethodSymbol(
                "PickUnaryNumericReturn",
                "string",
                ...
                [
                    new MetadataParameterSymbol("value", "int", MetadataByRefKind.None, IsParams: false, IsOptional: false),
                    new MetadataParameterSymbol("transform", "System.Func`2<int, string>", MetadataByRefKind.None, IsParams: false, IsOptional: false)
                ])
        ],
        [],
        [],
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda unary numeric expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate unary numeric return overload candidate.");
    AssertEqual("System.Func`2<int, int>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateIfReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickIfReturn("Ada", text => if text == "Ada" { "match" } else { "fallback" })
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda if-expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate if-expression return overload candidate.");
    AssertEqual("System.Func`2<string, string>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateBlockReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickBlockReturn("Ada", text => { (text) })
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
...
static void CSharpOverloadResolverFiltersLambdaDelegateCoalesceReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        ...
        fun choose(): string = LegacyDelegateOverloads.PickCoalesceReturn(LegacyNamed("Ada"), item => item.Name ?? "fallback")
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda null-coalescing expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate coalesce return overload candidate.");
    AssertEqual("System.Func`2<Legacy.Tools.LegacyNamed, string>", selected.Method.Parameters[1].Type);
}

static void CSharpOverloadResolverFiltersLambdaDelegateIndexerReturnType()
{
    var parseResult = TypeSharpParser.ParseText("""
        namespace Samples.OverloadResolver

        fun choose(): string = LegacyDelegateOverloads.PickIndexerReturn(LegacyFormatter("legacy:"), item => item[0])
        """);
    var root = Require(parseResult.Root, "Parser should produce a root syntax node.");
    var call = Require(FindFirstNode(root, SyntaxKind.CallExpression), "Test input should contain a call expression.");
    var arguments = call.Children.Skip(1).Where(child => !child.IsToken).ToArray();
    var legacyFormatter = new MetadataTypeSymbol(
        "Legacy.Tools",
        "LegacyFormatter",
        [],
        [
            new MetadataPropertySymbol("Item", "string", IsStatic: false, HasPublicGetter: true, HasPublicSetter: false, IsIndexer: true, ParameterCount: 1)
            {
                ParameterTypes = ["int"]
            }
        ],
    ...
    var metadataType = new MetadataTypeSymbol(
        ...
        [
            new MetadataMethodSymbol(
                ...
                MetadataNullabilityKind.NotApplicable,
                [
                    new MetadataParameterSymbol("value", "Legacy.Tools.LegacyFormatter", MetadataByRefKind.None, IsParams: false, IsOptional: false),
                    new MetadataParameterSymbol("transform", "System.Func`2<Legacy.Tools.LegacyFormatter, string>", MetadataByRefKind.None, IsParams: false, IsOptional: false)
                ]),
            new MetadataMethodSymbol(
                "PickIndexerReturn",
                "string",
                ...
                [
                    new MetadataParameterSymbol("value", "Legacy.Tools.LegacyFormatter", MetadataByRefKind.None, IsParams: false, IsOptional: false),
                    new MetadataParameterSymbol("transform", "System.Func`2<Legacy.Tools.LegacyFormatter, int>", MetadataByRefKind.None, IsParams: false, IsOptional: false)
                ])
        ],
        [],
        [],
    ...
    var assembly = new MetadataAssemblySymbol("Legacy.Tools", ResolvedReferenceKind.LocalAssembly, "Legacy.Tools", null, null)
    {
        Types = [legacyFormatter, metadataType]
    };
    ...
    AssertEqual(1, resolution.ApplicableCandidates.Count);
    AssertFalse(resolution.IsAmbiguous, "Lambda indexer expression return type should remove incompatible delegate overload candidates.");
    var selected = Require(resolution.SelectedCandidate, "Resolver should select the compatible delegate indexer return overload candidate.");
    AssertEqual("System.Func`2<Legacy.Tools.LegacyFormatter, string>", selected.Method.Parameters[1].Type);
}
...
static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaBinaryValueReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaBinaryValueReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaBinaryValueReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaBinaryValueReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresBinaryValueReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda binary value return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaBinaryValueReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda binary value return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaBinaryValueReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda binary value return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaNameofReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaNameofReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaNameofReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaNameofReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresNameofReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda nameof return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaNameofReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda nameof return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaNameofReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda nameof return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaUncheckedReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaUncheckedReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaUncheckedReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaUncheckedReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresUncheckedReturnString", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda unchecked return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaUncheckedReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda unchecked return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaUncheckedReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda unchecked return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaSatisfiesReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaSatisfiesReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaSatisfiesReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaSatisfiesReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresSatisfiesReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda satisfies return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaSatisfiesReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda satisfies return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaSatisfiesReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda satisfies return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaParenthesizedReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaParenthesizedReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaParenthesizedReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaParenthesizedReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresParenthesizedReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda parenthesized return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaParenthesizedReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda parenthesized return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaParenthesizedReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda parenthesized return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaLogicalNotReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaLogicalNotReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaLogicalNotReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaLogicalNotReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresLogicalNotReturnString", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda logical-not return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaLogicalNotReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda logical-not return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaLogicalNotReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda logical-not return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaUnaryNumericReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaUnaryNumericReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaUnaryNumericReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaUnaryNumericReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresUnaryNumericReturnString", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda unary numeric return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaUnaryNumericReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda unary numeric return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaUnaryNumericReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda unary numeric return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaIfReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaIfReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaIfReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaIfReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresIfReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda if-expression return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaIfReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda if-expression return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaIfReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda if-expression return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaBlockReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaBlockReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaBlockReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaBlockReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresBlockReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda block return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaBlockReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda block return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaBlockReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda block return overload diagnostics contain errors.");
    });
}
...
static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaCoalesceReturnOverload()
{
    WithWorkspace(root =>
    {
        ...
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaCoalesceReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaCoalesceReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaCoalesceReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresCoalesceReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda coalesce return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaCoalesceReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda coalesce return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaCoalesceReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda coalesce return overload diagnostics contain errors.");
    });
}

static void CliBuildStopsBeforeEmissionOnNoMatchingCSharpDelegateLambdaIndexerReturnOverload()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "NoMatchingDelegateLambdaIndexerReturnOverloadBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.NoMatchingDelegateLambdaIndexerReturnOverloadBuild"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.NoMatchingDelegateLambdaIndexerReturnOverloadBuild
        ...
        AssertEqual(string.Empty, output.ToString());
        AssertContains("\"code\": \"TS2406\"", error.ToString());
        AssertContains("LegacyDelegateOverloads.RequiresIndexerReturnInt", error.ToString());
        AssertContains("matches no overload candidate", error.ToString());
        AssertFalse(File.Exists(Path.Combine(root, "generated", "src", "Main.g.cs")), "Build should not emit generated C# when no matching delegate lambda indexer return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "NoMatchingDelegateLambdaIndexerReturnOverloadBuild.Generated.csproj")), "Build should not emit generated project when no matching delegate lambda indexer return overload diagnostics contain errors.");
        AssertFalse(File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "NoMatchingDelegateLambdaIndexerReturnOverloadBuild.dll")), "Build should not emit generated assembly when no matching delegate lambda indexer return overload diagnostics contain errors.");
    });
}
...
static void CliBuildCompilesParenthesizedExpressionLowering()
{
    WithWorkspace(root =>
    {
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ParenthesizedExpressionBuild"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ParenthesizedExpressionBuild"
            generatedOutputRoot = "generated"
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ParenthesizedExpressionBuild

            export fun sum(): int = (1 + 1)
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertEqual(0, exitCode);
        AssertContains("Generated assembly: bin/Debug/net48/ParenthesizedExpressionBuild.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("return (1 + 1);", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ParenthesizedExpressionBuild.dll")),
            "Generated project build should compile parenthesized expressions.");
    });
}
...
static void CliBuildCompilesImportedDelegateLambdaOverloadBinaryValueReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadBinaryValueReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadBinaryValueReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadBinaryValueReturnMatch

            import { LegacyDelegateOverloads, LegacyNamed } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickBinaryValueReturn(LegacyNamed("Ada"), item => item.Name + "!")
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload binary value return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadBinaryValueReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickBinaryValueReturn(new LegacyNamed(\"Ada\"), item => item.Name + \"!\");", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadBinaryValueReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload binary value return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadNameofReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadNameofReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadNameofReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadNameofReturnMatch

            import { LegacyDelegateOverloads, LegacyNamed } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickNameofReturn(LegacyNamed("Ada"), item => nameof(item.Name))
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload nameof return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadNameofReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickNameofReturn(new LegacyNamed(\"Ada\"), item => nameof(item.Name));", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadNameofReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload nameof return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadCheckedReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadCheckedReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadCheckedReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadCheckedReturnMatch

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickCheckedReturn("Ada", text => checked(1 + 1))
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload checked return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadCheckedReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickCheckedReturn(\"Ada\", text => checked(1 + 1));", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadCheckedReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload checked return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadSatisfiesReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadSatisfiesReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadSatisfiesReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadSatisfiesReturnMatch

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickSatisfiesReturn("Ada", text => text satisfies string)
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload satisfies return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadSatisfiesReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickSatisfiesReturn(\"Ada\", text => text);", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadSatisfiesReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload satisfies return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadParenthesizedReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadParenthesizedReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadParenthesizedReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadParenthesizedReturnMatch

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickParenthesizedReturn("Ada", text => (text))
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload parenthesized return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadParenthesizedReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickParenthesizedReturn(\"Ada\", text => (text));", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadParenthesizedReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload parenthesized return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadLogicalNotReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadLogicalNotReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadLogicalNotReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadLogicalNotReturnMatch

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickLogicalNotReturn(true, flag => !flag)
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload logical-not return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadLogicalNotReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickLogicalNotReturn(true, flag => !flag);", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadLogicalNotReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload logical-not return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadUnaryNumericReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadUnaryNumericReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadUnaryNumericReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadUnaryNumericReturnMatch

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickUnaryNumericReturn(3, value => -value)
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload unary numeric return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadUnaryNumericReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickUnaryNumericReturn(3, value => -value);", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadUnaryNumericReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload unary numeric return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadIfReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadIfReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadIfReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadIfReturnMatch

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickIfReturn("Ada", text => if text == "Ada" { "match" } else { "fallback" })
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload if-expression return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadIfReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickIfReturn(\"Ada\", text => (new System.Func<string>(() => { if (text == \"Ada\") { return \"match\"; } else { return \"fallback\"; } }))());", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadIfReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload if-expression return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadBlockReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadBlockReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadBlockReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadBlockReturnMatch

            import { LegacyDelegateOverloads } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickBlockReturn("Ada", text => { (text) })
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload block return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadBlockReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickBlockReturn(\"Ada\", text => { return (text); });", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadBlockReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload block return matches.");
    });
}
...
static void CliBuildCompilesImportedDelegateLambdaOverloadCoalesceReturnMatch()
{
    WithWorkspace(root =>
    {
        ...
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadCoalesceReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadCoalesceReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadCoalesceReturnMatch

            import { LegacyDelegateOverloads, LegacyNamed } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickCoalesceReturn(LegacyNamed("Ada"), item => item.Name ?? "fallback")
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload coalesce return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadCoalesceReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickCoalesceReturn(new LegacyNamed(\"Ada\"), item => item.Name ?? \"fallback\");", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadCoalesceReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload coalesce return matches.");
    });
}

static void CliBuildCompilesImportedDelegateLambdaOverloadIndexerReturnMatch()
{
    WithWorkspace(root =>
    {
        BuildLegacyReferenceDll(root, "Legacy.Tools");
        var manifestPath = WriteManifest(root, """
            [project]
            name = "ImportedDelegateLambdaOverloadIndexerReturnMatch"
            targetFramework = "net48"
            outputType = "library"
            rootNamespace = "Samples.ImportedDelegateLambdaOverloadIndexerReturnMatch"
            generatedOutputRoot = "generated"
            ...
            """);
        WriteFile(root, "src/Main.tysh", """
            namespace Samples.ImportedDelegateLambdaOverloadIndexerReturnMatch

            import { LegacyDelegateOverloads, LegacyFormatter } from "Legacy.Tools"

            export fun pick(): string = LegacyDelegateOverloads.PickIndexerReturn(LegacyFormatter("legacy:"), item => item[0])
            """);
        using var output = new StringWriter();
        using var error = new StringWriter();
        ...
        AssertTrue(exitCode == 0, $"Imported delegate lambda overload indexer return match should build.\nSTDOUT:\n{output}\nSTDERR:\n{error}");
        AssertContains("Generated assembly: bin/Debug/net48/ImportedDelegateLambdaOverloadIndexerReturnMatch.dll", output.ToString());
        AssertEqual(string.Empty, error.ToString());

        var generatedSource = File.ReadAllText(Path.Combine(root, "generated", "src", "Main.g.cs")).Replace("\r\n", "\n", StringComparison.Ordinal);
        AssertContains("using Legacy.Tools;", generatedSource);
        AssertContains("return LegacyDelegateOverloads.PickIndexerReturn(new LegacyFormatter(\"legacy:\"), item => item[0]);", generatedSource);
        AssertTrue(
            File.Exists(Path.Combine(root, "generated", "bin", "Debug", "net48", "ImportedDelegateLambdaOverloadIndexerReturnMatch.dll")),
            "Generated project build should compile imported delegate lambda overload indexer return matches.");
    });
}
    ...
    WriteFile(projectRoot, "LegacyApi.cs", """
        ...
        namespace Legacy.Tools
        {
            ...
            public static class LegacyDelegateOverloads
            {
                ...
                public static string PickBinaryValueReturn(LegacyNamed value, System.Func<LegacyNamed, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickBinaryValueReturn(LegacyNamed value, System.Func<LegacyNamed, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickNameofReturn(LegacyNamed value, System.Func<LegacyNamed, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickNameofReturn(LegacyNamed value, System.Func<LegacyNamed, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickCheckedReturn(string value, System.Func<string, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickCheckedReturn(string value, System.Func<string, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickUncheckedReturn(string value, System.Func<string, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickUncheckedReturn(string value, System.Func<string, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickSatisfiesReturn(string value, System.Func<string, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickSatisfiesReturn(string value, System.Func<string, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickParenthesizedReturn(string value, System.Func<string, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickParenthesizedReturn(string value, System.Func<string, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickLogicalNotReturn(bool value, System.Func<bool, bool> transform)
                {
                    return transform(value) ? "true" : "false";
                }

                public static string PickLogicalNotReturn(bool value, System.Func<bool, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickUnaryNumericReturn(int value, System.Func<int, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickUnaryNumericReturn(int value, System.Func<int, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickIfReturn(string value, System.Func<string, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickIfReturn(string value, System.Func<string, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickBlockReturn(string value, System.Func<string, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickBlockReturn(string value, System.Func<string, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }
                ...
                public static string PickCoalesceReturn(LegacyNamed value, System.Func<LegacyNamed, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }

                public static string PickIndexerReturn(LegacyFormatter value, System.Func<LegacyFormatter, string> transform)
                {
                    return "text:" + transform(value);
                }

                public static string PickIndexerReturn(LegacyFormatter value, System.Func<LegacyFormatter, int> transform)
                {
                    return "int:" + transform(value).ToString();
                }
                ...
                public static string RequiresBinaryValueReturnInt(LegacyNamed value, System.Func<LegacyNamed, int> transform)
                {
                    return transform(value).ToString();
                }

                public static string RequiresNameofReturnInt(LegacyNamed value, System.Func<LegacyNamed, int> transform)
                {
                    return transform(value).ToString();
                }

                public static string RequiresUncheckedReturnString(string value, System.Func<string, string> transform)
                {
                    return transform(value);
                }

                public static string RequiresSatisfiesReturnInt(string value, System.Func<string, int> transform)
                {
                    return transform(value).ToString();
                }

                public static string RequiresParenthesizedReturnInt(string value, System.Func<string, int> transform)
                {
                    return transform(value).ToString();
                }

                public static string RequiresLogicalNotReturnString(bool value, System.Func<bool, string> transform)
                {
                    return transform(value);
                }

                public static string RequiresUnaryNumericReturnString(int value, System.Func<int, string> transform)
                {
                    return transform(value);
                }

                public static string RequiresIfReturnInt(string value, System.Func<string, int> transform)
                {
                    return transform(value).ToString();
                }
                ...
                public static string RequiresIndexerReturnInt(LegacyFormatter value, System.Func<LegacyFormatter, int> transform)
                {
                    return transform(value).ToString();
                }
            }
