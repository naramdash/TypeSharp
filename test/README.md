...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 564
```
...
dotnet test `
  ...
  --minimum-expected-tests 568 `
  --no-progress
```

The `568` package-shard expectation is the 564 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
