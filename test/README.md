...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 574
```
...
dotnet test `
  ...
  --minimum-expected-tests 578 `
  --no-progress
```

The `578` package-shard expectation is the 574 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
