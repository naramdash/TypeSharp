...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 556
```
...
dotnet test `
  ...
  --minimum-expected-tests 560 `
  --no-progress
```

The `560` package-shard expectation is the 556 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
