...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 566
```
...
dotnet test `
  ...
  --minimum-expected-tests 570 `
  --no-progress
```

The `570` package-shard expectation is the 566 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
