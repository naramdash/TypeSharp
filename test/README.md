...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 572
```
...
dotnet test `
  ...
  --minimum-expected-tests 576 `
  --no-progress
```

The `576` package-shard expectation is the 572 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
