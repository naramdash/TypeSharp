...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 558
```
...
dotnet test `
  ...
  --minimum-expected-tests 562 `
  --no-progress
```

The `562` package-shard expectation is the 558 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
