...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 554
```
...
dotnet test `
  ...
  --minimum-expected-tests 558 `
  --no-progress
```

The `558` package-shard expectation is the 554 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
