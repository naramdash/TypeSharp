...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 552
```
...
dotnet test `
  ...
  --minimum-expected-tests 556 `
  --no-progress
```

The `556` package-shard expectation is the 552 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
