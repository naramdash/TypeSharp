...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 560
```
...
dotnet test `
  ...
  --minimum-expected-tests 564 `
  --no-progress
```

The `564` package-shard expectation is the 560 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
