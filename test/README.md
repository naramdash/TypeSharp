...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 586
```
...
dotnet test `
  ...
  --minimum-expected-tests 590 `
  --no-progress
```

The `590` package-shard expectation is the 586 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
