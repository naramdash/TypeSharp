...
dotnet test --project test\TypeSharp.Compiler.Tests.MSTest\TypeSharp.Compiler.Tests.MSTest.csproj --filter "FullyQualifiedName~CatalogCase" --minimum-expected-tests 562
```
...
dotnet test `
  ...
  --minimum-expected-tests 566 `
  --no-progress
```

The `566` package-shard expectation is the 562 shared catalog cases plus one `CatalogIsExposedForPackageRunners` smoke per shard.
