...
This page is the web-facing view of the long-running task ledger. The compact source record lives in `docs/tasks/README.md` and `docs/tasks/0001-0253-task-ledger-rollup.md`.
...
| Current tooling slice | Task `0253-cli-manifest-semantic-validation` is complete and folded into the compressed task rollup. TypeSharp manifests now reject unsupported option values for output type, language version, nullable mode, and diagnostic format before source discovery. |
...
| Compressed work ledger | [`docs/tasks/0001-0253-task-ledger-rollup.md`](https://github.com/naramdash/TypeSharp/blob/main/docs/tasks/0001-0253-task-ledger-rollup.md) |
...
| CLI and VS Code | `check`, `build`, `run`, `format`, `new`, common options, warning gates, target/configuration/verbosity handling, strict option parsing, semantic manifest value validation, VS Code activation, LSP diagnostics, hover, definition, completion, and formatting. |
