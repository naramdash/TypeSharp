...
This is the canonical docs-site example catalog and coverage guide. Example source artifacts live under the repository root `examples/` folder; their durable narrative and coverage index belongs here.
...
As of task `0251`, raw example source and runnable projects live outside `docs/` under `examples/`. docs-site owns the narrative catalog, while the root `examples/` tree remains the smoke-tested source/project artifact area.
