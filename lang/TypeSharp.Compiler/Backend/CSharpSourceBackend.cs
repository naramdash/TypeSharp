...
public static class CSharpSourceBackend
{
    ...
    private sealed class Emitter
    {
        ...
        private readonly List<CSharpExtensionProperty> _extensionProperties = [];
            ...
            IReadOnlyList<CSharpSourceFunctionReExport> functionReExports)
        {
            ...
            {
                RegisterExtensionProperties(extensionDeclarations[index], index);
            }
        ...
        private void EmitExtensionDeclaration(SyntaxNode node, int index)
        {
            ...
            var receiverSourceType = GetSourceTypeName(receiverTypeNode);
            var receiverName = GetExtensionReceiverIdentifier(node)?.Text ?? "receiver";
            ...
            var properties = node.Children
                .Where(child => child.Kind == SyntaxKind.ValueDeclaration && !IsAmbientDeclaration(child))
                .ToArray();
            ...
            {
                if (functions.Length > 0 || propertyIndex > 0)
                {
                    _builder.AppendLine();
                }

                EmitExtensionProperty(properties[propertyIndex], receiverType, receiverSourceType, receiverName);
            }
        ...
        private void EmitExtensionProperty(
            SyntaxNode node,
            string receiverType,
            string receiverSourceType,
            string receiverName)
        {
            var visibility = GetVisibility(node);
            var name = GetLocalDeclarationName(node);
            var helperName = GetExtensionPropertyHelperName(name);
            var initializer = GetInitializerExpression(node);
            var type = GetValueDeclarationType(node, initializer);
            var parameterName = string.IsNullOrWhiteSpace(receiverName) ? "receiver" : receiverName;

            var previousValueTypes = _valueTypes;
            _valueTypes = new Dictionary<string, string>(previousValueTypes, StringComparer.Ordinal)
            {
                [parameterName] = receiverSourceType
            };

            try
            {
                var value = initializer is null ? EmitDefaultValue(type) : EmitExpression(initializer, type);
                _builder.AppendLine($"        {visibility} static {type} {helperName}(this {receiverType} {parameterName})");
                _builder.AppendLine("        {");
        ...
        private void RegisterExtensionProperties(SyntaxNode node, int index)
        {
            var receiverTypeNode = GetExtensionReceiverTypeNode(node);
            var receiverSourceType = GetSourceTypeName(receiverTypeNode);
            if (string.IsNullOrWhiteSpace(receiverSourceType))
            {
                return;
            }

            var className = GetExtensionClassName(receiverTypeNode, index);
            foreach (var property in node.Children.Where(child => child.Kind == SyntaxKind.ValueDeclaration && !IsAmbientDeclaration(child)))
            {
                var name = GetLocalDeclarationName(property);
                if (string.IsNullOrWhiteSpace(name))
                {
                    continue;
                }

                var initializer = GetInitializerExpression(property);
                _extensionProperties.Add(new CSharpExtensionProperty(
                    receiverSourceType,
                    name,
                    GetValueDeclarationSourceType(property, initializer),
                    className,
                    GetExtensionPropertyHelperName(name)));
            }
        }
        ...
        {
            if (TryGetExtensionPropertyAccess(node, out var property, out var extensionReceiver))
            {
                return $"{property.ClassName}.{property.HelperName}({EmitExpression(extensionReceiver)})";
            }
        ...
        private string InferEmitterExpressionType(SyntaxNode? node)
        {
            ...
            {
                return extensionProperty.SourceType;
            }
        ...
        private bool TryGetExtensionPropertyAccess(
            SyntaxNode node,
            out CSharpExtensionProperty property,
            out SyntaxNode receiver)
        {
            property = default;
            receiver = default!;
            if (!TryGetMemberAccessParts(node, out receiver, out var memberName))
            {
                return false;
            }

            if (TryGetImportedMemberAccess(
                node,
                requireWritable: false,
                out _,
                out _,
            ...
            var receiverType = InferEmitterExpressionType(receiver);
            if (string.IsNullOrWhiteSpace(receiverType) ||
                string.Equals(receiverType, "object", StringComparison.Ordinal) ||
                receiverType.EndsWith("?", StringComparison.Ordinal))
            {
                return false;
            }

            if (TryGetKnownShape(receiverType, out var shape) &&
                shape.Parameters.Any(parameter => string.Equals(parameter.Name, memberName, StringComparison.Ordinal)))
            {
                return false;
            }

            foreach (var candidate in _extensionProperties)
            {
                if (string.Equals(candidate.Name, memberName, StringComparison.Ordinal) &&
                    string.Equals(receiverType, candidate.ReceiverSourceType, StringComparison.Ordinal))
                {
                    property = candidate;
                    return true;
                }
        ...
        private readonly record struct CSharpExtensionProperty(
            string ReceiverSourceType,
            string Name,
            string SourceType,
            string ClassName,
