...
Task 0251 이후 `docs-site/`는 표준 언어/프로젝트 원장의 canonical 표면이다. `docs/`는 agentic goal work를 위한 임시 운영 표면으로 남긴다. 현재 task 운영 구조는 [docs/tasks.md](docs/tasks.md)가 active task pointer와 다음 우선순위를 들고, [docs/tasks-rollup.md](docs/tasks-rollup.md)가 완료된 task history를 압축 보관하며, 진행 중인 긴 작업만 `docs/NNNN-short-name.md` active packet으로 남기는 방식이다. [Document Ownership](docs-site/src/content/docs/document-ownership.md)이 어떤 표준 문서를 docs-site canonical로 둘지와 `docs/`에 어떤 temporary work 문서만 남길지를 정한다. [Project Ledger](docs-site/src/content/docs/project-ledger.md), [Work Ledger](docs-site/src/content/docs/work-ledger.md), [Agentic Workflow](docs-site/src/content/docs/agentic-workflow.md)는 원장성 문서를 웹페이지로 보여준다. Codex CLI goal, Ralph mode, 장기 실행 에이전트가 다음 작업을 고를 때의 운영 입력은 이 파일, [docs/agentic-execution.md](docs/agentic-execution.md), [docs/checklist.md](docs/checklist.md), [docs/tasks.md](docs/tasks.md), [docs/tasks-rollup.md](docs/tasks-rollup.md), [docs/traceability.md](docs/traceability.md), 그리고 docs-site의 Document Ownership/Project Ledger/Work Ledger/Agentic Workflow다.
...
```text
TypeSharp 장기 작업을 시작할 때는 docs/agentic-execution.md의 부트스트랩 순서와 작업 선택 규칙을 따른다. 각 반복은 `docs/tasks.md`의 active task 또는 checklist/traceability의 다음 후보 하나를 선택하고, docs-site canonical 표준 문서와 docs temporary work 문서가 충돌하지 않게 문서, 사양, 코드, 테스트 중 필요한 산출물을 만든다. 완료 시 검증 결과와 남은 작업을 checklist.md, traceability.md, `docs/tasks-rollup.md`, 관련 docs-site canonical 문서에 반영하고, Done이 된 active task packet은 rollup에 압축한 뒤 제거한다. task가 Done이면 관련 변경을 git commit한 뒤 원격 브랜치로 push한다.
```
...
4. `docs/tasks.md`가 가리키는 active task packet이 있으면 그 파일
5. [docs/tasks-rollup.md](docs/tasks-rollup.md)
6. [docs/checklist.md](docs/checklist.md)
7. [docs/traceability.md](docs/traceability.md)
8. [docs-site Document Ownership](docs-site/src/content/docs/document-ownership.md)
9. [docs-site Project Ledger](docs-site/src/content/docs/project-ledger.md)
10. [docs-site Work Ledger](docs-site/src/content/docs/work-ledger.md)
11. [docs-site Agentic Workflow](docs-site/src/content/docs/agentic-workflow.md)
12. [docs-site Grammar](docs-site/src/content/docs/grammar.md)
13. [Grammar And Language Reference](docs-site/src/content/docs/reference.md)
14. [docs-site Feature Status](docs-site/src/content/docs/feature-status.md)
15. 현재 작업과 직접 관련된 docs-site canonical 문서 또는 `docs/` temporary work 문서
...
1. [docs/tasks.md](docs/tasks.md)를 매 반복 다시 읽고 `State`, `User Task Inbox`, `Agent Task Queue`를 확인한다.
2. `In Progress` active task packet이 있으면, 사용자 요청과 충돌하지 않는 한 그 작업을 먼저 이어간다.
3. 활성 task가 없으면 [docs/tasks.md](docs/tasks.md)의 `User Task Inbox` unchecked item을 먼저 `Agent Task Queue`로 승격한다.
4. user inbox가 비어 있으면 `Agent Task Queue`의 가장 높은 우선순위 `Requested` 또는 `Ready` 항목을 고른다.
5. 두 task 섹션이 모두 비어 있으면 [docs/checklist.md](docs/checklist.md)의 미완료 항목을 [docs/agentic-execution.md](docs/agentic-execution.md)의 Q0-Q5 규칙으로 고른다.
6. 완료된 큰 주제는 [docs/tasks-rollup.md](docs/tasks-rollup.md)를 먼저 읽고, 필요한 경우에만 현재 active packet을 연다. 완료된 개별 task packet은 `docs/`에 계속 쌓지 않는다.
7. task 0251 이후 docs-site는 표준 언어/프로젝트 원장의 canonical 표면이 되며, `docs/`는 agentic temporary work와 handoff 문서만 남긴다. 구현 우선순위와 활성 작업 상태는 `docs/tasks.md`, active task packet, `docs/tasks-rollup.md`가 결정하고, docs-site Work Ledger는 그 상태를 웹에서 보여준다.
