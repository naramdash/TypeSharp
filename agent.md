...
- 지금 작업이 기존 .NET Framework ASP.NET, WCF, Windows Service, worker-style host에서 일반 `net481` library처럼 참조/배포/로드되는 계약을 깨뜨릴 가능성이 있는가?
...
1. .NET Framework 4.8.1 및 기존 ASP.NET/WCF/worker host 호환성
...
3. C# library interop 구현 범위 확정: framework assembly, local DLL, overload, nullable, public ABI fixture
4. .NET Framework ASP.NET/WCF/worker compatibility contract를 generated assembly, runtime dependency, host smoke criteria로 구체화
5. C# metadata-backed interop validation: overload, nullable, invalid byref, public ABI fixture
6. Standard library core surface 확정: `TypeSharp.Core`, `TypeSharp.Collections`, `TypeSharp.Runtime`
7. C# reference resolver and metadata reader expansion
8. C# 7.3 source backend 구현 확대
9. CLI command 구현 순서와 project manifest validation 확정
10. VS Code extension/LSP 최소 구조 확정
11. nominal closed union 런타임 표현 선택
12. TypeScript식 type-level union의 public boundary 정책 확정
13. structural type public boundary 정책
14. MSBuild 통합 전략
15. IL backend 도입 조건 정의
