...
| Local DLL reference | MVP | `paths`로 명시한 `net481` 호환 DLL을 읽는다. |
| NuGet package reference | Stable Backlog | `net481` compatible asset, restore, lock file, license inventory가 필요하다. |
...
- public TypeSharp API가 C#으로 나갈 때 nullable metadata emit을 목표로 하되, `net481` compiler/toolchain 제약을 compatibility note에 남긴다.
...
- `net481`에서 로드할 수 없는 assembly/package를 참조함
...
2. TypeSharp가 local `net481` C# class library DLL을 참조하고 generic method, event, delegate를 호출한다.
3. C# `net481` console 또는 test project가 TypeSharp library assembly를 참조하고 public API를 호출한다.
