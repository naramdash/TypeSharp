...
| ASP.NET/WCF/worker host compatibility | MVP contract, Stable Backlog smokes | generated `net48` library와 runtime은 host-specific migration 없이 기존 .NET Framework application model에서 로드 가능해야 하며, templates and host smokes는 별도 단계로 둔다. |
...
2. TypeSharp가 local `net48` C# class library DLL을 참조하고 generic method, event, delegate를 호출한다.
