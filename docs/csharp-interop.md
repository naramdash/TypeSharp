...
| `dynamic`/reflection/COM/P/Invoke | MVP opt-in | `dynamic`, `reflect`, `interop`, `unsafe` marker 없이는 strict mode에서 diagnostic이다. `dynamic` type annotation without a `dynamic fun` boundary is reported as `TS2206`; calling a `dynamic fun` from a non-`dynamic` function is reported as `TS2207`. |
