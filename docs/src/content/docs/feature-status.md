...
| Pipeline and composition | MVP limited | First-argument pipeline and unary composition lower to C# 7.3-compatible calls and delegate lambdas. Direct pipeline targets with known TypeSharp signatures are checked for first-parameter input compatibility, lowered arity, non-piped call argument compatibility, and final-array `params` tails; direct generic TypeSharp pipeline targets also infer and substitute simple, array, matching single-argument wrapper, and implemented `params` tail type parameters from the piped input plus non-piped arguments. Known value-shaped `>>` and `<<` operands report `TS2201` instead of lowering as invalid composition. Direct named unary function pairs with known TypeSharp signatures are compatibility-checked, including bounded generic edge inference for direct TypeSharp generic unary targets, explicit unary function-type annotations on direct composition values validate the annotated input/result, and unannotated non-exported direct composition values infer concrete generated delegates only for fully known direct TypeSharp-declared unary signatures. Public/exported unannotated direct composition values report `TS2201`. Numeric shifts, higher-order function values, currying, partial application, broader/public composition function-type inference, imported pipeline/composition targets, pipeline overload ranking, and broader composition type inference remain backlog. |
