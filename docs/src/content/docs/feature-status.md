...
- Manifest-owned current-project source aliases are implemented for source graph imports/re-exports. TypeSharp project references remain accepted only when they resolve to generated `net48` assemblies and explicit export metadata, or report deterministic diagnostics before emission.
...
| ES module imports/exports and type-only imports | MVP limited | Relative source imports/re-exports, type-only imports, namespace imports, explicit export surfaces, and current-project manifest-owned source aliases are implemented for source modules. Alias targets must resolve into the TypeSharp source graph and generated C# identity, or diagnose before emission. |
