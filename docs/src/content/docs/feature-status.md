...
| Pipelines and composition | MVP limited | `|>`, `>>`, and `<<` are implemented with C# 7.3-compatible lowering. Direct `value |> f` and `value |> f(args...)` targets validate the piped input, lowered arity, and non-piped call arguments against known TypeSharp-declared parameter lists, with bounded inference/substitution for direct TypeSharp generic targets. Known value-shaped `>>` and `<<` operands report an ambiguity diagnostic because numeric shifts are not implemented, and direct unary named-function composition pairs validate known signatures including bounded generic TypeSharp edge inference. Broader partial-application/currying remains backlog because generated delegate shapes must stay predictable for C# consumers. |
