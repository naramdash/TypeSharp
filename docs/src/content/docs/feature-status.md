...
| Direct function calls | MVP limited | Direct TypeSharp-declared function calls validate known arity and argument types before lowering. A final `params name: T[]` parameter accepts either one exact array argument or expanded trailing element arguments, including zero trailing arguments after fixed parameters. Trailing literal defaults shaped `name: Type = literal` can be omitted from direct calls and first-argument pipeline lowering for non-generic direct `fun` declarations; supported defaults are string, numeric, `true`, `false`, and `null`, and public generated C# exposes ordinary optional parameter metadata. Direct generic TypeSharp function calls support explicit type arguments plus inference/substitution when declared type parameters appear directly as parameter/return types, inside bounded constructed shapes such as arrays and matching single-argument generic wrappers, or through the implemented final-array `params` tail. Imported C# calls stay on metadata-backed overload validation. Direct TypeSharp named argument binding for known TypeSharp-owned direct functions is the active next slice; optional/defaults on generic functions, ambient/`extern` signatures, or `params`, function-typed values, higher-order calls, currying, partial application, broader type-constructor unification, and TypeSharp function overload ranking remain backlog. |
