...
| Explicit-receiver extension members | MVP limited | TypeSharp-authored methods lower to C# extension methods. Getter-only extension properties lower to `GetName(this T receiver)` helper methods and member accesses call those helpers. Exact duplicate/conflict diagnostics, same-container helper-name collision diagnostics, nullable receiver declaration/access diagnostics, assignment-target diagnostics, and null-conditional `?.` read/simple assignment target diagnostics are implemented before backend emission. Nullable receiver lifting, richer conversion rules, setters, static extension members, operators, and imported extension property metadata remain backlog. |
