...
| Direct function calls | MVP limited | Direct TypeSharp-declared function calls validate known arity and argument types before lowering. A final `params name: T[]` parameter accepts either one exact array argument or expanded trailing element arguments, including zero trailing arguments after fixed parameters. Trailing literal defaults shaped `name: Type = literal` can be omitted from direct calls and first-argument pipeline lowering for non-generic direct `fun` declarations; supported defaults are string, numeric, `true`, `false`, and `null`, and public generated C# exposes ordinary optional parameter metadata. Known non-generic TypeSharp-owned direct calls may use named arguments after any positional prefix, and first-argument pipeline calls may name non-piped arguments; accepted calls lower to positional C# and invalid names/order/duplicates/missing required/type mismatches report `TS2201`. Direct generic TypeSharp function calls support explicit type arguments plus inference/substitution when declared type parameters appear directly as parameter/return types, inside bounded constructed shapes such as arrays and matching single-argument generic wrappers, or through the implemented final-array `params` tail when named arguments are not used. Imported C# calls stay on metadata-backed overload validation. Optional/defaults on generic functions, ambient/`extern` signatures, or `params`, named TypeSharp args on generic/`params` functions, function-typed values, higher-order calls, currying, partial application, broader type-constructor unification, and TypeSharp function overload ranking remain backlog. |
