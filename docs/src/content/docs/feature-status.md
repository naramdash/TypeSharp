...
| Direct function calls | MVP limited | Direct TypeSharp-declared function calls validate known arity and argument types before lowering. Direct generic TypeSharp function calls support explicit type arguments and simple inference/substitution when declared type parameters appear directly as parameter or return types. Imported C# calls stay on metadata-backed overload validation. Constructed generic parameter inference, generic pipeline/composition inference, optional/default/params TypeSharp parameter policy, function-typed values, higher-order calls, currying, partial application, and TypeSharp function overload ranking remain backlog. |
