...
| Pipelines and composition | MVP limited | `|>`, `>>`, and `<<` are implemented with C# 7.3-compatible lowering. Known value-shaped `>>` and `<<` operands report an ambiguity diagnostic because numeric shifts are not implemented. Broader partial-application/currying remains backlog because generated delegate shapes must stay predictable for C# consumers. |
