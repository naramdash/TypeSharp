...
TypeSharp-owned functions support two bounded convenience shapes. A final parameter may be written as `params name: T[]`; direct calls and first-argument pipeline lowering accept either an exact `T[]` argument or expanded trailing `T` arguments. Direct `fun` declarations may also give trailing parameters literal defaults with `name: Type = literal`, where supported defaults are string, numeric, `true`, `false`, and `null`; omitted direct-call and pipeline arguments use the generated C# optional parameter metadata. Defaulted parameters must form a suffix, cannot combine with `params`, and are not yet supported on generic functions, ambient/`extern` signatures, named TypeSharp arguments, constructors, delegates, union cases, function types, lambdas, or higher-order values. TypeSharp function overload ranking remains backlog; imported C# overloads continue to use the metadata-backed interop path.
...
| Pattern matching, direct TypeSharp-declared function call and bounded generic inference diagnostics, final-array `params`, trailing literal default parameters, pipeline, composition, composition/shift ambiguity diagnostics, direct function composition compatibility including bounded generic unary targets, explicit function-type annotation checks, public composition annotation-required checks, and non-exported direct composition value inference, direct pipeline input/arity/call-argument/bounded generic inference compatibility, async, `yield`, `lock`, extension methods, collections, `nameof`, checked/unchecked | [Grammar](../grammar/), [Feature Status](../feature-status/), [Lowering](../lowering/) | Parser snapshots, type checker fixtures, backend snapshots, generated build and C# consumer smokes. |
