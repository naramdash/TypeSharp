...
Control forms such as `if`, `match`, `try`, `using`, async blocks, `yield`, and `lock` use dedicated parse forms instead of infix precedence. Planned operators such as `**`, shifts, shift assignment, ranges, `===`, and `!==` are reserved or diagnostic-producing until stabilized; enum member initializer-local `|`, same-enum value expression `|`, `&`, `^`, unary `~`, integral numeric expression `|`, `&`, `^`, unary `~`, boolean expression `|`, `&`, `^`, and bitwise compound assignment `|=`, `&=`, `^=` are the implemented bitwise-shaped exceptions.
