...
- Simple enum declarations use `enum Name { Member, Other }`; members can include explicit integer numeric values such as `Member = 1`, and declarations can use explicit integral underlying types such as `enum Name : byte { Member = 1 }`. Computed member values, flag policy, enum aliases, and enum-member attributes remain planned.
