...
The implemented null-conditional grammar slice parses postfix member and indexer access so `receiver?.Member = value` and `receiver?[index] = value` can be recognized as simple assignment targets. Semantic support is intentionally narrower than parsing: null-conditional reads, compound assignment, increment/decrement, events, static targets, local binding targets, and TypeSharp-owned member/indexer targets remain follow-ups.
