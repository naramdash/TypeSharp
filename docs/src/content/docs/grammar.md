...
Control forms such as `if`, `match`, `try`, `using`, async blocks, `yield`, and `lock` use dedicated parse forms instead of infix precedence. Planned operators such as `**`, logical unsigned shift assignment, ranges, `===`, and `!==` are reserved or diagnostic-producing until stabilized; enum member initializer-local `|`, same-enum value expression `|`, `&`, `^`, unary `~`, integral numeric expression `|`, `&`, `^`, unary `~`, bounded integral `<<`/`>>`/`>>>`, boolean expression `|`, `&`, `^`, bitwise compound assignment `|=`, `&=`, `^=`, shift assignment `<<=`, `>>=`, and invalid value-shaped `>>`/`<<`/`>>>` diagnostics are the implemented bitwise-shaped exceptions.
