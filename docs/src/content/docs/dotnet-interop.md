...
- Public property get/set, field read/write, bounded null-conditional imported instance field/property/indexer assignment and read expressions, imported instance field/property `receiver?.Member += value`, `receiver?.Member -= value`, and `receiver?.Member >>>= count`, imported instance indexer `receiver?[index] += value`, `receiver?[index] -= value`, and `receiver?[index] >>>= count`, indexers, events, extension methods, delegates, imported enum match exhaustiveness, collection expression array arguments including single collection expressions in `params T[]` positions, optional/named/`params` parameters, parenthesized and unary numeric overload arguments, `ref`/`out`/`in`, generic methods, and generic constraints are validated for the implemented metadata subset before emission.
