...
- imported C# public static binary `operator *`, `operator /`, and `operator %` metadata for mutable local, regular readable/writable imported C# field/property member, and regular readable/writable imported C# indexer `*=`, `/=`, and `%=` assignment when exactly one operator matches and its result assigns back,
