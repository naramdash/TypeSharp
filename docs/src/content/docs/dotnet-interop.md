...
- Public property get/set, field read/write, indexers, events, extension methods, delegates, collection expression array arguments including single collection expressions in `params T[]` positions, optional/named/`params` parameters, `ref`/`out`/`in`, generic methods, and generic constraints are validated for the implemented metadata subset before emission.
