...
- imported C# public static binary `operator *`, `operator /`, and `operator %` metadata for mutable local and regular readable/writable imported C# field/property member `*=`, `/=`, and `%=` assignment when exactly one operator matches and its result assigns back,
