...
- properties and fields, including regular and checked/unchecked multiplicative compound assignment for readable/writable metadata-backed imported C# instance/static fields/properties over bounded integral, floating-point, decimal, and selected imported static binary operator operands plus bounded null-conditional assignment/read, additive compound assignment, multiplicative compound assignment over bounded integral/floating-point/decimal operands or selected imported static binary operators, checked/unchecked null-conditional member multiplicative assignment with guard-body overflow context, bitwise compound assignment, shift compound assignment, and logical unsigned shift assignment support for metadata-backed imported C# instance fields/properties,
- indexers, including regular bounded integral/floating-point/decimal and imported static binary user-defined multiplicative compound assignment, regular and null-conditional checked/unchecked bounded integral/floating-point/decimal or selected imported static binary multiplicative compound assignment, bounded null-conditional assignment/read, additive compound assignment, bounded integral/floating-point/decimal or selected imported static binary null-conditional multiplicative compound assignment, bitwise compound assignment, shift compound assignment, and logical unsigned shift assignment support for metadata-backed imported C# instance indexers,
...
- imported C# public static binary `operator *`, `operator /`, and `operator %` metadata for mutable local, regular readable/writable imported C# field/property member, regular readable/writable imported C# indexer, null-conditional readable/writable imported C# field/property member, and null-conditional readable/writable imported C# indexer `*=`, `/=`, and `%=` assignment when exactly one operator matches and its result assigns back,
...
- Public property get/set, field read/write, imported C# public static binary `operator *`, `operator /`, and `operator %` metadata for mutable local, regular readable/writable imported C# field/property member, and regular readable/writable imported C# indexer `*=`, `/=`, and `%=` assignment, imported field/property `receiver.Member *= value`, `receiver.Member /= value`, and `receiver.Member %= value` plus `checked(...)`/`unchecked(...)` wrappers for readable/writable metadata-backed instance/static targets over bounded integral, floating-point, decimal, and selected imported static binary operator operands, imported instance indexer `receiver[index] *= value`, `receiver[index] /= value`, and `receiver[index] %= value` plus `checked(...)`/`unchecked(...)` wrappers for metadata-backed getter/setter indexers over bounded integral, floating-point, decimal, and selected imported static binary operator operands plus supported arguments, bounded null-conditional imported instance field/property/indexer assignment and read expressions, imported instance field/property `receiver?.Member += value`, `receiver?.Member -= value`, `receiver?.Member *= value`, `receiver?.Member /= value`, `receiver?.Member %= value`, `receiver?.Member <<= count`, `receiver?.Member >>= count`, and `receiver?.Member >>>= count` where null-conditional member multiplicative targets use bounded integral/floating-point/decimal or selected imported static binary operands, `checked(...)`/`unchecked(...)` wrappers for those null-conditional member multiplicative targets, imported instance indexer `receiver?[index] += value`, `receiver?[index] -= value`, `receiver?[index] *= value`, `receiver?[index] /= value`, `receiver?[index] %= value`, `receiver?[index] <<= count`, `receiver?[index] >>= count`, and `receiver?[index] >>>= count` where null-conditional indexer multiplicative targets also use bounded integral/floating-point/decimal or selected imported static binary operands plus supported arguments, indexers, events, extension methods, delegates, imported enum match exhaustiveness, collection expression array arguments including single collection expressions in `params T[]` positions, optional/named/`params` parameters, parenthesized and unary numeric overload arguments, `ref`/`out`/`in`, generic methods, and generic constraints are validated for the implemented metadata subset before emission.
...
Do not commit generated `.exe` or `.dll` files unless a checked-in binary fixture is explicitly required.
