...
- imported C# public static binary `operator *`, `operator /`, and `operator %` metadata for mutable local `*=`, `/=`, and `%=` assignment when exactly one operator matches and its result assigns back,
