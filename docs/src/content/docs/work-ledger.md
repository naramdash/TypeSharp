...
| Current tooling slice | Task `0264-runtime-artifact-architecture-docs` is complete and folded into the compressed task rollup. No active task is selected. |
...
| Docs and adoption | Astro Starlight docs site, TypeScript docs configuration, `tysh` syntax-highlighted TypeSharp examples through the shared TextMate grammar, Mermaid-rendered architecture diagrams, GitHub Pages workflow, pinned current docs package dependencies, runnable examples, migration guide, docs benchmarks, host compatibility notes, progress/ADR/regression policy, release readiness, and traceability. |
