...
| Current tooling slice | Task `0263-docs-tysh-syntax-highlighting` is complete and folded into the compressed task rollup. No active task is selected. |
...
| Docs and adoption | Astro Starlight docs site, TypeScript docs configuration, `tysh` syntax-highlighted TypeSharp examples through the shared TextMate grammar, GitHub Pages workflow, pinned current docs package dependencies, runnable examples, migration guide, docs benchmarks, host compatibility notes, progress/ADR/regression policy, release readiness, and traceability. |
