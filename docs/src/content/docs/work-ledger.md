...
| Current tooling slice | Task `0262-vs-code-syntax-highlighting-extension-install-guide` is complete and folded into the compressed task rollup. No active task is selected. |
...
| CLI and VS Code | `check`, `build`, `run`, `format`, `new`, `lsp`, common options, warning gates, target/configuration/verbosity handling, strict option parsing, semantic manifest value validation, VS Code activation, TextMate syntax highlighting for `.tysh`, local VSIX packaging/install docs, temporary Marketplace publishing guide, LSP diagnostics, hover, definition, completion, and formatting. |
| Docs and adoption | Astro Starlight docs site, TypeScript docs configuration, GitHub Pages workflow, pinned current docs package dependencies, runnable examples, migration guide, docs benchmarks, host compatibility notes, progress/ADR/regression policy, release readiness, and traceability. |
