...
| `record` | Public ABI slice | Named immutable CLR class with declaration attributes, `partial` when declared, constructor/properties plus generated equality/hash members. | Backend snapshots and C# consumer smokes cover immutable records, declaration attribute metadata, partial modifier preservation, record updates, and record expression construction. |
