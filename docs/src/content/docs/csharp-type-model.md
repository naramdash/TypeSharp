...
| `class` | Public ABI slice, MVP limited | Named CLR class with optional generic parameters/constraints, `partial`, an implicit public parameterless constructor, public instance/static `fun` methods, typed instance/static `let` and `let mut` values, and typed instance/static `event` members backed by named delegate types. | Class API, generic type, generic constraint, partial declaration, instance/static method members, instance/static value members, instance/static event members, unsupported member diagnostic, and C# consumer smokes cover the 1.0 subset. TypeSharp-authored constructors, properties, explicit inheritance/implementation clauses, static member forms beyond class static methods/values/events, abstract/virtual/override members, setters, indexers, operators, nested types, partial methods, and broader member-body analysis remain backlog. |
