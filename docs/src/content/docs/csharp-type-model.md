...
| `union` | Public ABI slice, MVP limited | Named abstract CLR base type with declaration attributes, nested sealed case types, and runtime helper metadata. | Nominal union API, declaration attribute, union match lowering, runtime helper, and C# consumer smokes cover the current class-hierarchy representation. Alternative representations and richer recursive ergonomics remain backlog. |
