...
| `event` | Public ABI slice, MVP limited | CLR event member on a generated class using a named delegate type. | Class event backend snapshots and generated `net48` C# consumer smokes cover subscription to generated class event metadata. Interface events, custom add/remove accessors, event invocation helpers, static events, and nullable receiver/event access remain backlog. |
...
| Callback or event handler | TypeSharp-authored `public delegate`, TypeSharp-authored class `event`, imported C# delegate/event, or explicit function type annotation | inferred function type or interface/custom/static event declarations before event lowering evidence |
