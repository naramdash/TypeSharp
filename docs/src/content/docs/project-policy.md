...
- NuGet.org account setup, package ownership, API key creation, and first publish remain release-owner tasks documented in `agent/nuget-deploy.md`. The tag-oriented `release-artifacts.yml` workflow intentionally stops after pack and local install smoke; main-branch package publication is handled by `.github/workflows/nuget-publish-main.yml` when repository secret `NUGET_API_KEY` is configured.
