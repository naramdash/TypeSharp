...
- The next CI adoption slice should exercise the package-free four-shard runner as the release-confidence path and the MSTest bridge as a focused package-based discovery smoke. Full MSTest catalog execution should remain optional unless its extra runtime provides distinct evidence beyond the shard runner.
