...
- GitHub [`actions/runner-images`](https://github.com/actions/runner-images), [`actions/setup-dotnet`](https://github.com/actions/setup-dotnet), [`actions/setup-node`](https://github.com/actions/setup-node), and the [GitHub Actions image-migration changelog](https://github.blog/changelog/2026-05-14-github-actions-upcoming-image-migrations/): `windows-latest` is a supported GitHub-hosted Windows label and the setup actions support the .NET 10 and Node 24 workflow shape. The June 2026 Windows Server 2025 + Visual Studio 2026 image migration is a CI-watch item, not a generated-artifact baseline change. Tasks 0402, 0404, 0406, 0408, and 0410 confirmed the current GitHub Actions failure is not a setup-node/setup-dotnet availability failure or a missing NuGet test package; it is tracked as Task 0401 and awaits explicit approval for a C# process-launch fix around `npm`.
