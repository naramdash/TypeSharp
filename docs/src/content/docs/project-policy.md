...
Official .NET/NuGet/VS Code and .NET test-platform ecosystem sources refreshed and reaffirmed on 2026-05-23, including the Task 0399 refresh after null-conditional member assignment landed, the Task 0402 refresh after imported indexer null-conditional assignment landed, the Task 0404 refresh after getter-only extension properties landed, the Task 0406 refresh after extension-property duplicate/conflict diagnostics landed, the Task 0408 refresh after extension-property helper-name collision diagnostics landed, the Task 0410 refresh after nullable extension-property receiver diagnostics landed, the Task 0412 refresh after extension-property assignment diagnostics landed, the Task 0414 refresh after nullable extension-property receiver access diagnostics landed, the Task 0416 refresh after extension-property null-conditional access diagnostics landed, the Task 0418 refresh after imported C# null-conditional member reads landed, the Task 0420 refresh after imported C# null-conditional indexer reads landed, the Task 0422 refresh after imported C# null-conditional logical unsigned shift assignment member targets landed, the Task 0424 refresh after imported C# null-conditional logical unsigned shift assignment indexer targets landed, the Task 0426 refresh after imported C# null-conditional bitwise compound assignment member targets landed, the Task 0428 refresh after imported C# null-conditional bitwise compound assignment indexer targets landed, the Task 0430 refresh after imported C# null-conditional additive compound assignment member targets landed, the Task 0432 refresh after imported C# null-conditional additive compound assignment indexer targets landed, the Task 0434 refresh after imported C# null-conditional shift compound assignment member targets landed, the Task 0436 refresh after imported C# null-conditional shift compound assignment indexer targets landed, the Task 0438 refresh after local multiplicative compound assignment expressions landed, the Task 0440 refresh after imported C# member multiplicative compound assignment targets landed, the Task 0442 refresh after imported C# indexer multiplicative compound assignment targets landed, the Task 0444 refresh after imported C# null-conditional member multiplicative compound assignment targets landed, the Task 0446 refresh after imported C# null-conditional indexer multiplicative compound assignment targets landed, the Task 0448 refresh after local floating-point and decimal multiplicative compound assignment policy landed, the Task 0450 refresh after imported C# regular member floating-point and decimal multiplicative compound assignment policy landed, the Task 0452 refresh after imported C# regular indexer floating-point and decimal multiplicative compound assignment policy landed, the Task 0454 refresh after imported C# null-conditional member floating-point and decimal multiplicative compound assignment policy landed, the Task 0456 refresh after imported C# null-conditional indexer floating-point and decimal multiplicative compound assignment policy landed, the Task 0458 refresh after local multiplicative compound assignment checked/unchecked overflow policy landed, the Task 0460 refresh after imported C# regular member multiplicative compound assignment checked/unchecked overflow policy landed, the Task 0462 refresh after imported C# regular indexer multiplicative compound assignment checked/unchecked overflow policy landed, the Task 0464 refresh after imported C# null-conditional member multiplicative compound assignment checked/unchecked overflow policy landed, the Task 0466 refresh after imported C# null-conditional indexer multiplicative compound assignment checked/unchecked overflow policy landed, the Task 0468 refresh after imported C# static binary user-defined multiplicative operator policy landed, the Task 0470 refresh after imported C# regular member user-defined multiplicative operator policy landed, the Task 0472 refresh after imported C# regular indexer user-defined multiplicative operator policy plus the Task 0401 CI process-launch fix landed, and the Task 0474 refresh after imported C# null-conditional member user-defined multiplicative operator policy landed:
...
- Microsoft Learn [.NET test platforms overview](https://learn.microsoft.com/en-us/dotnet/core/testing/test-platforms-overview), [`dotnet test` with MTP](https://learn.microsoft.com/en-us/dotnet/core/testing/unit-testing-with-dotnet-test), [MSTest runner guidance](https://learn.microsoft.com/en-us/dotnet/core/testing/unit-testing-mstest-running-tests), [MSTest SDK configuration](https://learn.microsoft.com/en-us/dotnet/core/testing/unit-testing-mstest-sdk), NuGet [`MSTest.Sdk`](https://www.nuget.org/packages/MSTest.Sdk), NuGet [`xunit.v3`](https://www.nuget.org/packages/xunit.v3), xUnit.net [MTP guidance](https://xunit.net/docs/getting-started/v3/microsoft-testing-platform), NUnit [MTP guidance](https://docs.nunit.org/articles/vs-test-adapter/NUnit-And-Microsoft-Test-Platform.html), NuGet [`NUnit`](https://www.nuget.org/packages/NUnit), and NuGet [`NUnit3TestAdapter`](https://www.nuget.org/packages/NUnit3TestAdapter): MTP/MSTest SDK, xUnit.net v3, and NUnit are valid modern test-host choices. TypeSharp currently uses a pinned MSTest SDK bridge for package-based discovery while keeping the package-free main/shard runners as the release-confidence path; Task 0474 rechecked NuGet `MSTest.Sdk/4.2.3`, stable `xunit.v3` 3.2.2 plus prerelease 4.0.0-pre.108, NUnit 4.6.1, and NUnit3TestAdapter 6.2.0, kept MSTest.Sdk/MTP as the current package path, and preserved the native MTP `--test-modules` module-parallelism minimum at 578 tests.
- GitHub [`actions/runner-images`](https://github.com/actions/runner-images), [`actions/setup-dotnet`](https://github.com/actions/setup-dotnet), [`actions/setup-node`](https://github.com/actions/setup-node), and the [Windows Server 2025 + Visual Studio 2026 migration issue](https://github.com/actions/runner-images/issues/14017): `windows-latest` is a supported GitHub-hosted Windows label, setup-dotnet sets up requested SDKs, and setup-node sets up requested Node.js versions and npm caching/authentication. The Windows Server 2025 + Visual Studio 2026 migration window beginning 2026-06-08 and completing 2026-06-15 remains an open CI-watch item, not a generated-artifact baseline change. Tasks 0402, 0404, 0406, 0408, 0410, 0412, 0414, 0416, 0418, 0420, 0422, 0424, 0426, 0432, 0434, 0436, 0438, 0440, 0442, 0444, 0446, 0448, 0450, 0452, 0454, 0456, 0458, 0460, 0462, 0464, 0466, 0468, and 0470 confirmed the GitHub Actions failure was not a setup-node/setup-dotnet availability failure or a missing NuGet test package; Task 0401 then fixed the C# test-runner process launch by routing `npm` commands through `cmd.exe /d /s /c` on Windows while keeping non-Windows `npm` execution direct. The first post-fix regression run `26324313960` succeeded and passed the previously failing `Run shard runners in parallel` step; post-Task-0473 Docs run `26326367484` and Regression run `26326367485` also succeeded before Task 0474 selected the next slice. The VS Code stable release API reports 1.121.0, while the 1.122 release-notes page exists as an editor/tooling watch signal; neither changes the generated-artifact baseline.
