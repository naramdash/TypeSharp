...
- The release build injects `TypeSharpBuildMetadata=<tag>` and `TypeSharpSourceRevision=<short commit>` into `TypeSharp.Compiler`; `typesharp version` must expose those values in text and JSON so support can identify the exact artifact a user downloaded.
...
- After publication, the workflow downloads the just-published GitHub Release CLI and runtime assets from the hosted release URL, verifies them against `SHA256SUMS.txt`, extracts `typesharp.cmd` and the runtime DLL layout, checks `typesharp version --json` against the release page metadata, and builds a clean `HostedReleaseSmoke` console project from outside the repository.
