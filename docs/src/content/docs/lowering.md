...
This page is the docs canonical lowering ledger. The former repository bridge file `docs/lowering.md` was removed after its durable content was folded here.
...
| Records | Immutable sealed C# classes with declaration attributes, constructor, get-only properties, equality, and hash code. |
| Enums, assignment, and bitwise operators | Ordinary C# enum declarations; explicit integral underlying types lower as C# enum base types; explicit integer member values, aliases, and enum initializer-local composite values such as `ReadWrite = Read \| Write` lower as C# enum assignments; enum member access remains `EnumName.Member`; same-enum value `\|`, `&`, `^`, and unary `~` expressions lower as C# `\|`, `&`, `^`, and `~`; primitive integral `<<` and `>>` shifts lower as C# shift operators when the right count is C# 7.3-compatible; mutable local identifier assignments lower as ordinary C# assignments after `let mut` and known compatibility checks; multiplicative compound assignments lower as C# `*=`, `/=`, and `%=` when mutable local operands, readable/writable metadata-backed imported C# instance/static field/property member targets, metadata-backed imported C# instance indexer targets with selected public getter/setter pairs plus supported arguments, imported C# null-conditional instance field/property member targets, or imported C# null-conditional instance indexer targets with selected public getter/setter pairs plus supported arguments satisfy the bounded integral/floating-point/decimal assign-back policy; mutable local, regular imported C# field/property member/indexer targets, and null-conditional imported C# field/property member/indexer targets can also lower through ordinary C# compound assignment or guard-body compound assignment when one selected imported static binary operator supplies the result; local and imported regular member statement-form `checked(... *= ...)`, `checked(... /= ...)`, `checked(... %= ...)`, `unchecked(... *= ...)`, `unchecked(... /= ...)`, and `unchecked(... %= ...)` lower as C# 7.3 `checked { ... }` or `unchecked { ... }` statement blocks for mutable local targets and readable/writable metadata-backed imported C# field/property member targets, while value-position checked/unchecked expressions keep `checked(expr)` or `unchecked(expr)`; bitwise compound assignments lower as C# `\|=`, `&=`, and `^=`; shift assignments lower as C# `<<=` and `>>=` when mutable local operands satisfy the primitive integral shift policy; logical unsigned shift assignment `>>>=` lowers for mutable local primitive integral targets, readable/writable metadata-backed imported C# field/property member targets, and metadata-backed imported C# instance indexer targets with a selected public getter/setter as explicit C# 7.3-compatible assignment/cast forms with no emitted `>>>` or `>>>=`; bounded null-conditional imported assignment `receiver?.Member = value` and `receiver?[index] = value` plus bounded imported reads `receiver?.Member` and `receiver?[index]` lower through explicit `System.Func` null/index guards; bounded null-conditional imported member/indexer additive `receiver?.Member += value`, `receiver?.Member -= value`, `receiver?[index] += value`, and `receiver?[index] -= value`, bounded null-conditional imported member/indexer multiplicative `receiver?.Member *= value`, `receiver?.Member /= value`, `receiver?.Member %= value`, `receiver?[index] *= value`, `receiver?[index] /= value`, and `receiver?[index] %= value`, bounded null-conditional imported member/indexer bitwise `receiver?.Member |= value`, `receiver?.Member &= value`, `receiver?.Member ^= value`, `receiver?[index] |= value`, `receiver?[index] &= value`, and `receiver?[index] ^= value`, bounded null-conditional imported member/indexer shift `receiver?.Member <<= count`, `receiver?.Member >>= count`, `receiver?[index] <<= count`, and `receiver?[index] >>= count`, plus member/indexer `>>>=`, lower through explicit null/index guards, evaluate right-side/index arguments/count only when the receiver is non-null, and emit no C# `?.`, `?[]`, `>>>`, or `>>>=`; value-type reads use nullable C# result types such as `int?`. |
| Nominal unions | Abstract base type with declaration attributes plus sealed case types and runtime pattern helper metadata. |
...
| Explicit-receiver extension members | Methods lower to C# extension methods with `this` on the receiver parameter. Getter-only extension properties lower to C# 7.3-compatible `GetName(this T receiver)` helper methods, and TypeSharp member access lowers to static helper calls. Nullable receiver declarations, helper-name collisions in the same emitted extension container, assignment targets that resolve to getter-only extension properties, nullable receiver accesses before lifting is implemented, and null-conditional `?.` reads or simple assignment targets for TypeSharp-authored extension properties are diagnosed before backend emission. |
...
Statement-form `checked(...)` and `unchecked(...)` multiplicative compound assignment now covers mutable local targets, readable/writable imported C# regular field/property member targets, readable/writable imported C# regular indexer targets with supported arguments, readable/writable imported C# null-conditional field/property member targets, and readable/writable imported C# null-conditional indexer targets with supported arguments. Null-conditional member wrappers lower to an immediately invoked C# 7.3 block lambda that returns on a null receiver and places the checked/unchecked assignment body inside the non-null branch, so the overflow context is not lost across a nested lambda body. Null-conditional indexer wrappers use an outer receiver guard plus an inner index-argument guard whose body contains the checked/unchecked assignment, so index arguments and the right side are still skipped when the receiver is null.
...
Records lower to immutable C# classes with declaration attributes, constructor parameters, get-only properties, value equality, and hash code. Record construction, record update, and nominal record spread lower to constructor calls in record parameter order.
...
Local identifier assignments lower only after the checker confirms the target is a `let mut` binding and validates known simple assignment compatibility; local multiplicative compound assignments `*=`, `/=`, and `%=` additionally validate known non-null primitive integral numeric operands, the bounded local `float`, `double`, and `decimal` policy, or one selected imported C# public static binary `operator *`, `operator /`, or `operator %`, then require the result to be assignable back before lowering directly to ordinary C# compound assignment operators. Regular readable/writable imported C# field/property member targets, regular imported C# indexer targets with selected public getter/setter pairs plus supported arguments, null-conditional imported C# field/property member targets, and null-conditional imported C# indexer targets use the same selected imported static binary operator policy. The local/member/indexer floating/decimal and imported static-operator slices keep generated source C# 7.3-compatible by emitting ordinary `*=`, `/=`, and `%=` over accepted locals, regular members, regular indexers, null-conditional member guard bodies, and null-conditional indexer guard bodies; mixed decimal-floating operands, nullable operands, missing or ambiguous imported static operators, instance-compound-only metadata, unsupported target shapes, missing or ambiguous indexer targets, and narrowing/non-assignable results stop before emission. Readable/writable metadata-backed imported C# instance/static field/property member targets, metadata-backed imported C# instance indexer targets with selected public getter/setter pairs plus supported arguments, null-conditional imported C# instance field/property member targets, and null-conditional imported C# instance indexer targets with selected public getter/setter pairs plus supported arguments reuse that bounded floating/decimal assign-back policy and lower to ordinary C# compound assignment inside their respective guard shapes. Statement-form `checked(...)` and `unchecked(...)` wrappers over mutable local, readable/writable imported C# regular field/property member, readable/writable imported C# regular indexer, readable/writable imported C# null-conditional field/property member, and readable/writable imported C# null-conditional indexer multiplicative assignments lower to C# 7.3 checked/unchecked blocks or guard-body checked/unchecked blocks. Member receivers and indexer receiver/index arguments appear once in generated source, preserving C# compound-assignment single-evaluation behavior; null-conditional indexer arguments are evaluated only inside the non-null branch. Bitwise compound assignments `|=`, `&=`, and `^=` validate known local enum/integral/bool operands before lowering directly to C# assignment operators; shift assignments `<<=` and `>>=` validate known local primitive integral targets and int-compatible counts before lowering directly to C# assignment operators. Logical unsigned shift assignment `>>>=` validates the same mutable local primitive integral target/count set and lowers with explicit assignment/cast forms such as `result = unchecked((int)(unchecked((uint)(result)) >> count))`, `result = unchecked((long)(unchecked((ulong)(result)) >> count))`, or `result = unchecked((byte)(unchecked((uint)(result)) >> count))`; unsigned `uint`/`ulong` targets use ordinary `>>` on the unsigned value. Readable/writable metadata-backed imported C# instance/static field and property member `>>>=` targets use the same explicit cast strategy; simple receivers lower as ordinary assignments, and non-trivial receivers lower through a generated `System.Func<TReceiver,TMember>` expression so the receiver expression is evaluated once before getter/setter use. Metadata-backed imported C# instance indexer `>>>=` targets with a selected public getter/setter use the same cast strategy; simple receiver/index argument shapes lower as ordinary indexer assignments, and non-trivial receiver or index arguments lower through a generated `System.Func<...>` expression so the receiver and each index argument are evaluated once before getter/setter use. Generated C# never emits `>>>` or `>>>=`. Imported C# event, unresolved member/indexer, and unsupported indexer `>>>=` targets remain rejected before emission.

Null-conditional assignment `receiver?.Member = value` for metadata-backed imported C# instance fields/properties lowers to an immediately invoked `System.Func<TReceiver,TMember>` whose body checks the receiver for `null`, returns `default(TMember)` when null, and evaluates `value` only inside the non-null assignment branch. Null-conditional additive compound assignment `receiver?.Member += value` and `receiver?.Member -= value` for readable/writable metadata-backed imported C# instance fields/properties uses the same guard shape, returns `default(TMember)` when the receiver is null, evaluates `value` only inside the non-null branch, and assigns through ordinary C# `+=` or `-=` over the implemented primitive integral target/value policy. Null-conditional multiplicative compound assignment `receiver?.Member *= value`, `receiver?.Member /= value`, and `receiver?.Member %= value` for readable/writable metadata-backed imported C# instance fields/properties uses the same guard shape, returns `default(TMember)` when the receiver is null, evaluates `value` only inside the non-null branch, and assigns through ordinary C# `*=`, `/=`, or `%=` over the bounded integral/floating-point/decimal assign-back policy or one selected imported C# public static binary operator whose result assigns back to the member value type. Null-conditional bitwise compound assignment `receiver?.Member |= value`, `receiver?.Member &= value`, and `receiver?.Member ^= value` for readable/writable metadata-backed imported C# instance fields/properties uses the same guard shape, returns `default(TMember)` when the receiver is null, evaluates `value` only inside the non-null branch, and assigns through ordinary C# compound assignment operators over the existing primitive integral, enum, and bool target/value policy. Null-conditional shift compound assignment `receiver?.Member <<= count` and `receiver?.Member >>= count` for readable/writable metadata-backed imported C# instance fields/properties uses the same guard shape, returns `default(TMember)` when the receiver is null, evaluates `count` only inside the non-null branch, and assigns through ordinary C# `<<=` or `>>=` over the existing primitive target/count policy. Null-conditional logical unsigned shift assignment `receiver?.Member >>>= count` for readable/writable metadata-backed imported C# instance fields/properties uses the same guard shape, returns `default(TMember)` when the receiver is null, evaluates `count` only inside the non-null branch, and assigns the existing C# 7.3-compatible unsigned-cast shift value in that branch. Null-conditional member reads `receiver?.Member` for readable metadata-backed imported C# instance fields/properties lower to an immediately invoked `System.Func<TReceiver,TResult>` null guard; value-type member reads use nullable C# result types such as `int?` and `default(int?)`, while reference-like member reads use the C# reference type and `default(TReference)`.

Null-conditional assignment `receiver?[index] = value` for metadata-backed imported C# instance indexers lowers through an outer `System.Func<TReceiver,TIndexerValue>` null guard plus an inner `System.Func<TIndex,TIndexerValue>` index-capture branch; the receiver is evaluated once, index arguments and `value` are evaluated only after the receiver is non-null, and generated C# never emits `?[]`. Null-conditional additive compound assignment `receiver?[index] += value` and `receiver?[index] -= value` for readable/writable metadata-backed imported C# instance indexers uses the same outer/inner guard shape, evaluates index arguments and `value` only after the receiver is non-null, and assigns through ordinary C# `+=` or `-=` over the implemented primitive integral target/value policy. Null-conditional multiplicative compound assignment `receiver?[index] *= value`, `receiver?[index] /= value`, and `receiver?[index] %= value` for readable/writable metadata-backed imported C# instance indexers uses the same outer/inner guard shape, evaluates index arguments and `value` only after the receiver is non-null, and assigns through ordinary C# `*=`, `/=`, or `%=` over the bounded integral/floating-point/decimal assign-back policy or one selected imported C# public static binary operator whose result assigns back to the indexer value type. Statement-form `checked(...)` and `unchecked(...)` wrappers around those null-conditional indexer multiplicative assignments lower to a C# 7.3 block lambda whose outer receiver guard returns `default(TIndexerValue)` on null and whose inner index-capture lambda contains the checked/unchecked assignment body, so index arguments and the right side are still skipped on null receivers. Null-conditional bitwise compound assignment `receiver?[index] |= value`, `receiver?[index] &= value`, and `receiver?[index] ^= value` for readable/writable metadata-backed imported C# instance indexers uses the same outer/inner guard shape, evaluates index arguments and `value` only after the receiver is non-null, and assigns through ordinary C# compound assignment operators over the existing primitive integral, enum, and bool target/value policy. Null-conditional shift compound assignment `receiver?[index] <<= count` and `receiver?[index] >>= count` for readable/writable metadata-backed imported C# instance indexers uses the same outer/inner guard shape, evaluates index arguments and `count` only after the receiver is non-null, and assigns through ordinary C# `<<=` or `>>=` over the existing primitive target/count policy. Null-conditional logical unsigned shift assignment `receiver?[index] >>>= count` for readable/writable metadata-backed imported C# instance indexers uses the same outer/inner guard shape, assigns the existing unsigned-cast shift value, evaluates index arguments and `count` only after the receiver is non-null, and emits no `?[]`, `>>>`, or `>>>=`. Null-conditional reads `receiver?[index]` for readable metadata-backed imported C# instance indexers use the same outer/inner guard shape, return nullable C# result types such as `int?` for value-type indexers, evaluate index arguments only after the receiver is non-null, and emit no `?[]`. This makes supported statement and expression positions valid C# 7.3. Other null-conditional compound assignment beyond the bounded member/indexer additive, multiplicative, bitwise, regular shift, and `>>>=` slices, increment/decrement, invocation, chains, events, static targets, local binding targets, and TypeSharp-owned member/indexer targets remain rejected before emission. Exhaustive enum matches over TypeSharp-owned enums and named imported C# enums lower to ordered `object.Equals` comparisons against generated or imported enum members. Imported C# enum underlying/numeric metadata is captured for future interop decisions, but current lowering remains name/member based. Unary boolean complement, arbitrary/general computed enum member values, flag algebra beyond same-enum value `|`/`&`/`^`/`~`, broad attribute target validation, numeric pattern algebra, and flag-style reasoning over imported numeric metadata are not lowered yet.

Public class, interface, generic type, generic function, TypeSharp-authored named delegate, TypeSharp-authored class/interface event, and delegate-compatible function value surfaces lower only when their CLR shape is explicit. TypeSharp-authored record, union, class, interface, and named delegate declaration attributes lower to C# attributes on the generated type declaration, and supported member attributes lower to C# attributes on emitted class/interface methods, values, properties, and events. TypeSharp-authored class constructor parameter lists with explicit CLR-visible parameter types lower to ordinary C# constructors with empty bodies; classes without parameter lists keep the C# implicit parameterless constructor. TypeSharp-authored class methods lower as instance or static methods according to the class member modifier. Immutable class `let` and `static let` values with explicit types and initializers lower to C# `readonly` and `static readonly` fields. Mutable class `let mut` and `static let mut` values with explicit types and initializers lower to C# mutable instance and static fields. Getter-only class `let name: Type = value { get }` and `static let name: Type = value { get }` properties lower to C# get-only auto-properties with explicit initializers. Mutable class `let mut name: Type = value { get set }` and `static let mut name: Type = value { get set }` properties lower to C# get/set auto-properties with explicit initializers. Getter-only interface `let name: Type { get }` properties lower to C# interface get-only property signatures, and mutable interface `let mut name: Type { get set }` properties lower to C# interface get/set property signatures. TypeSharp-authored named delegates lower to generated CLR delegate metadata with explicit parameter and return shapes. TypeSharp-authored class/interface events lower to ordinary C# event metadata backed by named delegate types; class events may lower as instance or static events, while interface events remain instance-only in the C# 7.3-compatible slice. `partial` is preserved for declarations that lower to generated C# type declarations: modules, records, unions, classes, and interfaces.
...
- `test/fixtures/backend/csharp/positive/0056-delegate-declaration-api`
- `test/fixtures/backend/csharp/positive/0057-class-event-api`
- `test/fixtures/backend/csharp/positive/0058-interface-event-api`
- `test/fixtures/backend/csharp/positive/0059-class-static-event-api`
- `test/fixtures/backend/csharp/positive/0060-class-static-method-api`
- `test/fixtures/backend/csharp/positive/0061-class-static-value-api`
- `test/fixtures/backend/csharp/positive/0062-class-instance-value-api`
- `test/fixtures/backend/csharp/positive/0063-class-mutable-value-api`
- `test/fixtures/backend/csharp/positive/0064-class-constructor-api`
- `test/fixtures/backend/csharp/positive/0065-class-getter-property-api`
- `test/fixtures/backend/csharp/positive/0066-class-setter-property-api`
- `test/fixtures/backend/csharp/positive/0067-interface-getter-property-api`
- `test/fixtures/backend/csharp/positive/0068-interface-setter-property-api`
- `test/fixtures/backend/csharp/positive/0069-class-interface-attribute-api`
- `test/fixtures/backend/csharp/positive/0070-class-interface-member-attribute-api`
...
Parenthesized expressions preserve grouping in generated C#. Metadata validation uses the enclosed expression type for implemented imported overload and indexer argument checks. Unary logical-not and numeric sign expressions lower directly to C# `!expr`, `+expr`, or `-expr`. Ordinary `nameof` targets lower to C# `nameof(...)`; unbound generic type targets such as `nameof(Box<>)` and `nameof(Pair<,>)` lower to string constants so generated `net48` source remains C# 7.3-compatible. `checked` and `unchecked` expressions lower directly to C# 7.3-compatible intrinsics, while accepted local, imported regular member, and imported regular indexer multiplicative assignment statements lower to checked/unchecked blocks. Explicit-receiver extension methods lower to C# extension methods in a static helper container. Getter-only TypeSharp extension properties lower to ordinary static helper methods such as `GetWordCount(this string text)`, and `value.WordCount` lowers to `StringExtensions.GetWordCount(value)` instead of emitting C# 14 extension-member syntax. A nullable receiver declaration, nullable receiver access before lifting is implemented, assignment target, null-conditional `?.` read or simple assignment target, or property helper name that would collide with a TypeSharp-authored extension method or another generated property helper in the same container reports `TS2213` before this backend path runs.
...
- C# interop CLI smokes in the compiler test catalog
