...
| Enums, assignment, and bitwise operators | Ordinary C# enum declarations; explicit integral underlying types lower as C# enum base types; explicit integer member values, aliases, and enum initializer-local composite values such as `ReadWrite = Read \| Write` lower as C# enum assignments; enum member access remains `EnumName.Member`; same-enum value `\|`, `&`, `^`, and unary `~` expressions lower as C# `\|`, `&`, `^`, and `~`; primitive integral `<<` and `>>` shifts lower as C# shift operators when the right count is C# 7.3-compatible; mutable local identifier assignments lower as ordinary C# assignments after `let mut` and known compatibility checks; bitwise compound assignments lower as C# `\|=`, `&=`, and `^=`; shift assignments lower as C# `<<=` and `>>=` when mutable local operands satisfy the primitive integral shift policy; logical unsigned shift assignment `>>>=` lowers for mutable local primitive integral targets as explicit C# 7.3-compatible assignment/cast forms with no emitted `>>>` or `>>>=`. |
