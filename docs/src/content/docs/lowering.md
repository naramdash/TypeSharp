...
Simple enum declarations lower to ordinary generated C# enum declarations. Explicit integral underlying types such as `: byte` are preserved as C# enum base types, and explicit integer member values such as `Red = 1` are preserved as C# enum member assignments. Member references such as `Color.Green` stay as direct C# enum member access. Exhaustive enum matches over TypeSharp-owned enums and named imported C# enums lower to ordered `object.Equals` comparisons against generated or imported enum members. Computed enum member values, flags, enum aliases, enum member attributes, imported C# enum underlying metadata, and numeric range validation are not lowered yet.
