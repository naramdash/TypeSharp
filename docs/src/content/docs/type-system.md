...
Direct TypeSharp-declared function calls are checked when the target has a known parameter list. For `f(args...)`, the supplied argument count must match the declared parameter count and each known argument type must be assignable to the corresponding parameter type. Direct pipeline compatibility is checked when the pipeline target is a TypeSharp-declared function with a known parameter list. For `value |> f` and `value |> f(args...)`, `value` must be assignable to `f`'s first parameter, and the lowered call must supply the same number of arguments as the known target parameter list. Non-piped call arguments are checked against the target parameters after the first pipeline input parameter before lowering to `f(value, args...)`. Direct named unary function composition is checked when both TypeSharp-declared function signatures are known: for `f >> g`, `f` must return a type assignable to `g`'s single parameter, and for `g << f`, `f` must return a type assignable to `g`'s single parameter. These checks keep common F#-style function/pipeline/composition mistakes deterministic without committing to higher-order function inference, imported/generic function inference, optional/default/params TypeSharp parameter policy, currying, partial application, pipeline overload ranking, TypeSharp function overload ranking, or multi-parameter composition.
