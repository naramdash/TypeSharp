...
- richer exhaustiveness remains a functional correctness priority; known nominal unions and local type-level unions report missing cases/members, `_` can cover the remainder, guarded arms do not prove coverage without an unguarded cover, and bool/enum cases remain next;
