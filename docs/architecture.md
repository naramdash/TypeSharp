...
- `textDocument/hover`는 open document text를 compiler parser/binder로 분석하고 identifier 위치의 bound symbol을 markdown hover content와 zero-based range로 반환한다.
- `textDocument/definition`은 hover와 같은 open document symbol lookup을 재사용하고 bound source symbol 선언 위치를 LSP Location으로 반환한다.
- `textDocument/completion`은 open document binder symbols, built-in types, core keywords를 deterministic completion items로 반환한다.
- workspace manifest discovery, project reference-aware diagnostics, project-wide completion, and metadata member completion은 후속 LSP 작업으로 남긴다.
