...
| --- | --- | --- |
| Goal mode | 장기 목표 문장, 금지선, 반복 루프, 완료 기준 | [../agent.md](../agent.md), docs-site [Core Goal](../docs-site/src/content/docs/goal.md), 이 문서 |
...
규칙:
- 작업 선택, 완료 판단, task 상태는 [checklist.md](checklist.md), [tasks.md](tasks.md), [traceability.md](traceability.md), 관련 task rollup이 결정한다.
...
- goal 실행 중 docs-site와 `docs/` 운영 문서가 충돌하면 [Document Ownership](../docs-site/src/content/docs/document-ownership.md)의 target owner를 기준으로 삼고, 같은 작업에서 운영 문서를 맞춘다.
...
1. [../agent.md](../agent.md)
2. [../docs-site/src/content/docs/goal.md](../docs-site/src/content/docs/goal.md)
...
- 큰 결정을 내리면 docs-site canonical page와 [traceability.md](traceability.md), [checklist.md](checklist.md) 중 필요한 곳에 연결한다.
...
장기 실행 모드가 작업을 시작할 때는 아래 형태로 내부 작업 단위를 만든다. 실제 파일로 남겨야 할 정도로 큰 작업은 `docs/NNNN-topic.md` 형태의 루트 파일을 만들고 이 양식을 사용한다.
...
- public ABI, backend, runtime, compiler API, CLI/LSP 계약에 영향을 주는 큰 결정은 [adr.md](adr.md)의 ADR 형식으로 남긴다.

Task packet 압축 기준:
- 큰 주제의 연속 작업이 완료되면 관련 task packet을 하나의 root-level rollup 문서로 압축할 수 있다.
...
현재 작업은 고정 목록으로 복사하지 않는다. 먼저 [tasks.md](tasks.md)의 active task를 이어가고, active task가 없으면 [checklist.md](checklist.md)의 미완료 항목을 고른다.
