...
| 작업 패킷 | 통과 | [tasks/README.md](tasks/README.md)가 foundation부터 public ABI snapshot smoke까지 완료된 task packet과 rollup index를 제공한다. 최근 패킷은 [tasks/0090-null-safety-diagnostics.md](tasks/0090-null-safety-diagnostics.md), [tasks/0091-basic-structural-type-checking.md](tasks/0091-basic-structural-type-checking.md), [tasks/0092-async-task-interop-lowering.md](tasks/0092-async-task-interop-lowering.md), [tasks/0093-lowering-examples-catalog.md](tasks/0093-lowering-examples-catalog.md), [tasks/0094-test-coverage-checklist-audit.md](tasks/0094-test-coverage-checklist-audit.md), [tasks/0095-progress-and-adr-policy.md](tasks/0095-progress-and-adr-policy.md), [tasks/0096-migration-guide-draft.md](tasks/0096-migration-guide-draft.md), [tasks/0097-stable-grammar-parser-fixture-audit.md](tasks/0097-stable-grammar-parser-fixture-audit.md), [tasks/0098-public-abi-snapshot-smoke.md](tasks/0098-public-abi-snapshot-smoke.md)다. |
...
| Binder | 통과 | `TypeSharpBinder`가 built-in types, namespace/import/type/function/value/literal/module declarations, union cases, type parameters, function parameters, local `let`, lambda parameters, for-pattern bindings, and match-pattern bindings를 scoped symbol table에 등록하고 simple type/value references를 resolution한다. `BinderBindsLocalDeclarationsWithoutDiagnostics`, `CheckerReportsUnresolvedNameDiagnostics`, `BinderFixtureDiagnosticsMatch`, backend fixture prechecks, and semantic model/LSP smokes가 current binder scope를 검증한다. Project-wide symbol graph, overload binding, duplicate symbol diagnostics, and flow analysis remain separate checklist items or future work. |
...
| Type checker | 통과 | `TypeSharpTypeChecker`가 current implemented syntax에 대해 explicit annotation mismatch, nullability contract, nominal union exhaustiveness, type-level union public boundary and narrowing, basic structural shape assignment/member access, and async `Task<T>` result checks를 수행한다. `TypeCheckerFixtureDiagnosticsMatch`, `TypeCheckerAcceptsBasicAnnotations`, `CheckerReportsTypeMismatchDiagnostics`, CLI JSON smokes for `TS2201`/`TS2202`/structural/public boundary diagnostics, build no-emission smokes, backend fixture prechecks, and LSP diagnostics smokes가 current type checker scope를 검증한다. Generic constraints and full flow analysis remain separate checklist items or future work. |
...
| C# interop tests | 통과 | `tests/TypeSharp.Compiler.Tests`가 framework/local reference resolution, local metadata indexing, byref/params/optional/named overload validation, unknown nullability diagnostics, imported constructor/static/instance/property calls, delegate lambda calls, event add/remove calls, generated TypeSharp assembly consumption from C#, and ASP.NET/WCF/worker-style `net48` host references를 smoke tests로 검증한다. |
| C# reference resolver for framework/local DLL | 통과 | `TypeSharpReferenceResolver`가 manifest framework assembly names를 deduplicated `FrameworkAssembly` references로 정규화하고 local DLL paths를 full path plus manifest-relative slash-normalized path로 정규화한다. `ReferenceResolverNormalizesFrameworkAssemblies`, `ReferenceResolverNormalizesLocalDllPaths`, and `ReferenceResolverReportsMissingLocalDllDiagnostics`가 duplicate framework assembly filtering, local DLL identity/path shape, and missing local DLL `TS2401` diagnostics를 검증한다. NuGet restore, GAC probing, and transitive package resolution은 현재 scope 밖이다. |
| C# metadata reader | 통과 | `TypeSharpMetadataReader`가 `ReferenceResolutionResult`를 소비해 framework references를 deterministic placeholders로 만들고 local `net48` DLL의 public top-level type, method, property, parameter, byref, `params`, optional parameter, and unknown nullability metadata를 `MetadataAssemblySymbol` graph로 index한다. `MetadataReaderCreatesFrameworkAssemblyPlaceholders`, `MetadataReaderCreatesLocalAssemblyPlaceholders`, `MetadataReaderIndexesLocalPublicSymbols`, `MetadataReaderPreservesReferenceResolutionDiagnostics`, and `MetadataReaderReportsMissingLocalMetadataInputs`가 현재 metadata reader scope를 검증한다. Full CLR metadata coverage, nested/private/internal members, generic constraints, and attribute payload decoding are out of current scope. |
