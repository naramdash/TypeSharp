...
| Type checker | 통과 | `TypeSharpTypeChecker`가 current implemented syntax에 대해 explicit annotation mismatch, nullability contract, nominal union exhaustiveness, type-level union public boundary and narrowing, basic structural shape assignment/member access, and async `Task<T>` result checks를 수행한다. `TypeCheckerFixtureDiagnosticsMatch`, `TypeCheckerAcceptsBasicAnnotations`, `CheckerReportsTypeMismatchDiagnostics`, CLI JSON smokes for `TS2201`/`TS2202`/structural/public boundary diagnostics, build no-emission smokes, backend fixture prechecks, and LSP diagnostics smokes가 current type checker scope를 검증한다. Full inference engine, C# overload resolution, generic constraints, and full flow analysis remain separate checklist items or future work. |
