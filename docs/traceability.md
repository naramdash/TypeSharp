...
| Backend abstraction | 통과 | `ITypeSharpBackend` defines the compiler backend seam with backend name, generated source extension, and syntax-root emission. `CSharpSourceBackendAdapter` wraps the current C# 7.3 source backend and `TypeSharpBuilder` emits generated source through the interface. `BackendAbstractionExposesCSharpSourceBackend`, backend snapshots, generated `net48` build smokes, and existing CLI build smokes verify the abstraction preserves current C# output. Direct IL backend and richer multi-artifact backend contracts remain separate work. |
