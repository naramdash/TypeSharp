...
| Diagnostics system | 통과 | [diagnostics.md](diagnostics.md)가 diagnostic code range, descriptor metadata, explanation surface, and golden fixture policy를 정의한다. `Diagnostic`, `SourceSpan`, `DiagnosticDescriptors`, and `DiagnosticJsonFormatter` implement 1-based source spans, CLI text shape, descriptor metadata, and JSON output. `DiagnosticDescriptorRegistryIsStable`, `DiagnosticTextUsesCliShape`, parser/binder/type-checker golden diagnostics, CLI JSON smokes, build no-emission smokes, and LSP diagnostics smokes verify current user-facing diagnostic surfaces. `typesharp explain`, suppressions, related information, localization, and fix suggestions remain future work. |
